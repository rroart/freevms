...
static inline unsigned short isapnp_read_word(unsigned char idx)
{
    return 0xffff;
}
static inline unsigned int isapnp_read_dword(unsigned char idx)
{
    return 0xffffffff;
}
...
static inline int isapnp_activate_dev(struct pci_dev *dev, const char *name)
{
    return -ENODEV;
}
