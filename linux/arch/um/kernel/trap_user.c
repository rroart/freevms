...
static void sleeping_process_signal(int pid, int sig)
{
	...
	/* If an xterm changes size and the process handling SIGWINCH is
	 * asleep, its signal needs to be annulled and the SIGWINCH needs 
	 * to be passed to the current running process.
	 * XXX if the current process is switching out, then the SIGWINCH
	 * might hit the wrong process.
	 */
	case SIGWINCH:
		if(ptrace(PTRACE_CONT, pid, 0, 0) < 0)
			tracer_panic("sleeping_process_signal : Failed to "
				     "annull SIGWINCH pid %d, errno = %d\n",
				     pid, sig);
		kill(external_pid(cpu_tasks[0].task), SIGWINCH);
		break;
