...
void vms___global_restore_flags(unsigned long flags)
{
	switch (flags) {
	case 0:
		__global_cli();
...
inline void vms_disable_irq_nosync(unsigned int irq)
{
...
static DECLARE_MUTEX(vms_probe_sem);
...
void vms_init_irq_proc (void)
{
	...
	entry->nlink = 1;
	entry->data = (void *)&vms_prof_cpu_mask;
