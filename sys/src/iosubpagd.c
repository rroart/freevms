 ...
 */
int ioc$verify_chan(unsigned short chan, struct _ccb ** ccbp)
{
    struct _ccb * ccb = NULL;

    if ((chan == 0) || (chan >= CHANNELCNT))
    {
        return SS$_IVCHAN;
    }
    ccb = &ctl$gl_ccbbase[chan];
...
int ioc_std$trandevnam(void * descr_p, int flags, char *buf, int *outlen, void **out_p)
{
    ...
    *out_p = buf;
    ...
    if ((sts & 1) == 0)
        return sts;
    found: *outlen = (ret.mylnmb)->lnmb$l_lnmx->lnmx$l_xlen;
