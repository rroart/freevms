...
int ec$readblk(struct _irp * i, struct _pcb * p, struct _ucb * u, struct _ccb * c);
...
void ec$struc_init (struct _crb * crb, struct _ddb * ddb, struct _idb * idb, struct _orb * orb, struct _ucb * ucb)
{
...
void ec$struc_reinit (struct _crb * crb, struct _ddb * ddb, struct _idb * idb, struct _orb * orb, struct _ucb * ucb)
{
    ddb->ddb$ps_ddt=&ec$ddt;
    //dpt_store_isr(crb,nl_isr);
    return;
}

int ec$unit_init (struct _idb * idb, struct _ucb * ucb)
{
    ucb->ucb$v_online = 0;
...
struct _dpt ec$dpt;
struct _ddb ec$ddb;
struct _ucbnidef ec$ucb ;
struct _crb ec$crb;

int ec$init_tables()
{
    ini_dpt_name(&ec$dpt, "ECDRIVER");
    ini_dpt_adapt(&ec$dpt, 0);
    ini_dpt_defunits(&ec$dpt, 1);
    ini_dpt_ucbsize(&ec$dpt,sizeof(struct _ucbnidef));
    ini_dpt_struc_init(&ec$dpt, ec$struc_init);
    ini_dpt_struc_reinit(&ec$dpt, ec$struc_reinit);
    ini_dpt_ucb_crams(&ec$dpt, 1/*NUMBER_CRAMS*/);
    ini_dpt_end(&ec$dpt);

    ini_ddt_unitinit(&ec$ddt, ec$unit_init);
    ini_ddt_start(&ec$ddt, startio);
    ini_ddt_cancel(&ec$ddt, ioc_std$cancelio);
    ini_ddt_end(&ec$ddt);

    /* for the fdt init part */
    /* a lot of these? */

    ini_fdt_act(&ec$fdt,IO$_READLBLK,ec$readblk,1);
    ini_fdt_act(&ec$fdt,IO$_READPBLK,ec$readblk,1);
    ini_fdt_act(&ec$fdt,IO$_READVBLK,ec$readblk,1);
    ini_fdt_act(&ec$fdt,IO$_WRITELBLK,ec$writeblk,1);
    ini_fdt_act(&ec$fdt,IO$_WRITEPBLK,ec$writeblk,1);
    ini_fdt_act(&ec$fdt,IO$_WRITEVBLK,ec$writeblk,1);
    ini_fdt_act(&ec$fdt,IO$_SETMODE,lan$setmode,1);
    ini_fdt_act(&ec$fdt,IO$_SETCHAR,lan$setchar,1);
    ini_fdt_act(&ec$fdt,IO$_SENSEMODE,lan$sensemode,1);
    ini_fdt_act(&ec$fdt,IO$_SENSECHAR,lan$sensechar,1);
    ini_fdt_end(&ec$fdt);

    return SS$_NORMAL;
}

long ec_iodb_vmsinit(long dev)
{
#if 0
    struct _ucb * ucb=&ec$ucb;
    struct _ddb * ddb=&ec$ddb;
    struct _crb * crb=&ec$crb;
#endif
    ...
    init_ucb(&ec$ucb, &ec$ddb, &ec$ddt, &ec$crb);
    init_crb(&ec$crb);
#endif
    ...
    init_ddb(ddb,&ec$ddt,ucb,"eca");
    init_ucb(ucb, ddb, &ec$ddt, crb);
    init_crb(crb);

//  ioc_std$clone_ucb(&er$ucb,&ucb);
    ec$init_tables();
    ec$struc_init (crb, ddb, idb, orb, ucb);
    ec$struc_reinit (crb, ddb, idb, orb, ucb);
    ec$unit_init (idb, ucb);
...
int ec_vmsinit(long dev)
{
    ...
    ddb=ec_iodb_vmsinit(dev);

    /* for the fdt init part */
    /* a lot of these? */

    ec_iodbunit_vmsinit(ddb,0,&u0);
...
vortex_open(struct net_device *dev)
{
    ...
    if (vp->full_bus_master_rx)   /* Boomerang bus master. */
    {
        ...
        for (i = 0; i < RX_RING_SIZE; i++)
        {
            ...
            buf = kmalloc(PKT_BUF_SZ, GFP_KERNEL);
            vp->rx_skbuff[i] = buf;
#if 0
            if (skb == NULL)
                break;          /* Bad news!  */
...
static int
vortex_start_xmit(struct _irp * i, struct _pcb * p, struct _ucb * u, struct _ccb * c)
{
    ...
    outl(i->irp$l_qio_p2 + 14, ioaddr + TX_FIFO);
    char * buf = lan$alloc_xmit_buf(i,u,dev->dev_addr);
    if (vp->bus_master)
    {
        /* Set the bus-master controller to transfer the packet. */
        int len = (i->irp$l_qio_p2 + 14 + 3) & ~3;
        outl(   vp->tx_skb_dma = pci_map_single(vp->pdev, buf, len, PCI_DMA_TODEVICE),
    ...
    else
    {
        ...
        outsl(ioaddr + TX_FIFO, buf, (i->irp$l_qio_p2 + 14 + 3) >> 2);
...
static int
boomerang_start_xmit(struct _irp * i, struct _pcb * p, struct _ucb * u, struct _ccb * c)
{
    struct _ucbnidef * ni = u;
    struct net_device *dev=ni->ucb$l_extra_l_1;
    struct vortex_private *vp = (struct vortex_private *)dev->priv;
...
#endif
                kfree(vp->tx_skb); /* Release the transferred buffer */
                if (inw(ioaddr + TxFree) > 1536)
                {
                    /*
...
boomerang_rx(struct net_device *dev)
{
    ...
    for (; vp->cur_rx - vp->dirty_rx > 0; vp->dirty_rx++)
    {
        ...
        {
            skb = kmalloc(PKT_BUF_SZ,GFP_KERNEL);
            if (skb == NULL)
            {
    ...
    if (vp->full_bus_master_rx)   /* Free Boomerang bus master Rx buffers. */
    {
        for (i = 0; i < RX_RING_SIZE; i++)
            if (vp->rx_skbuff[i])
            {
                ...
                // check dev_kfree_skb(vp->rx_skbuff[i]);
                kfree(vp->rx_skbuff[i]);
                vp->rx_skbuff[i] = 0;
            }
    }
