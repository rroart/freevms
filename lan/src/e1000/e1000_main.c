...
/*static*/
int e1000_xmit_frame(struct _irp * i, struct _pcb * p, struct _ucb * u, struct _ccb * c);
...
/*static*/
void e1000_tx_timeout(struct net_device *dev);
...
#if 0
// check not yet supported by freevms
...
e1000_tx_map(struct e1000_adapter *adapter, struct e1000_tx_ring *tx_ring,
             struct _irp * irp, unsigned int first, unsigned int max_per_txd,
             unsigned int nr_frags, unsigned int mss)
{
    struct e1000_buffer *buffer_info;
    unsigned int len = irp->irp$l_qio_p2;
    unsigned int offset = 0, size, count = 0, i;
#ifdef MAX_SKB_FRAGS
    unsigned int f;
    len -= irp->irp$l_qio_p2;
#endif
    ...
    while (len)
    {
        ...
        if (unlikely(adapter->pcix_82544 &&
                     !((unsigned long)(irp->irp$l_qio_p1 + offset + size - 1) & 4) &&
                     size > 4))
            size -= 4;
...
#define PKT_BUF_SZ           1536
    struct sk_buff * skb = kmalloc(PKT_BUF_SZ,GFP_KERNEL);
#endif
...
// freevms needs other interface
#define TXD_USE_COUNT(S, X) (((S) >> (X)) + 1 )
/*static*/ int
e1000_xmit_frame(struct _irp * i, struct _pcb * p, struct _ucb * u, struct _ccb * c)
{
    struct _ucbnidef * ni = u;
    struct net_device *netdev=ni->ucb$l_extra_l_1;
    struct e1000_adapter *adapter = netdev_priv(netdev);
    struct e1000_tx_ring *tx_ring;
    unsigned int first, max_per_txd = E1000_MAX_DATA_PER_TXD;
    unsigned int max_txd_pwr = E1000_MAX_TXD_PWR;
    unsigned int tx_flags = 0;
    unsigned int len = i->irp$l_qio_p2 + 14;
    unsigned long flags;
...
#if 0
    // check not yet supported in freevms?
    ...
    e1000_tx_queue(adapter, tx_ring, tx_flags,
                   e1000_tx_map(adapter, tx_ring, i, first,
...
#endif
        // freevms change
        char * buf = rx_desc->buffer_addr;
        int pkt_len = rx_desc->length;
...
copydone:
        ...
        // freevms change
        char * buf = rx_desc->read.buffer_addr;
        int pkt_len = rx_desc->wb.middle.length0;
