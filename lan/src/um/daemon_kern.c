...
static unsigned short daemon_protocol(u, buf)
{
...
static int daemon_read(int fd, char * buf, int mtu,
...
static int daemon_write(int fd, char * buf, int len,
			struct uml_net_private *lp)
{
	return(daemon_user_write(fd, buf, len,
