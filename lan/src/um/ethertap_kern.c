...
static unsigned short etap_protocol(u, buf)
{
...
static int etap_read(int fd, char * buf, int mtu, struct uml_net_private *lp)
{
	int len;

	len = net_recvfrom(fd, buf,
			   mtu + 2 * ETH_HEADER_ETHERTAP);
	if(len <= 0) return(len);
	len -= 2;
	return(len);
}

static int etap_write(int fd, char * buf, int len, struct uml_net_private *lp)
{
