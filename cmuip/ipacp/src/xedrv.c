...
static// DRV$Device_Info is a list of everything we want the IPACP
...
// Supply four receive buffers to device controller
    ...
    for (I = 0; I <= (MAX_RCV_BUF - 1); I++)
    {
        ...
        RC = sys$qio(ASTEFN, XE_chan, IO$_READVBLK, &Buff->XERCV$vms_code, XE_receive, XE_Int, &Buff->XERCV$data,
        DRV$MAX_PHYSICAL_BUFSIZE, 0, 0, rcvhdrs[I].XERCV$buf, 0);
        if (RC != SS$_NORMAL)
        {
...
// get ARP buffer and issue a receive qio
    ...
    IO$_READVBLK, &ARbuf->ar_ios0, xe_arprcv, XE_Int, ARbuf->ar_data, ARP_MAX_LEN, 0, 0, ARbuf->phys$1, 0);
    if (RC != SS$_NORMAL)
    {
...
// Issue the startup command to controller
    ...
    if (!((RC == SS$_NORMAL) && (IOS->xe$vms_code == SS$_NORMAL)))
    {
        if (IOS->xe$vms_code == SS$_BADPARAM)
            XE$ERR(XE_Int, "XE startup failure, RC=!XL,VMS_code=!XL,Param=!XL", RC, IOS->xe$vms_code, ((long *) IOS)[1]);
        else
            XE$ERR(XE_Int, "XE startup failure, RC=!XL,VMS_code=!XL,Xfer size=!SL", RC, IOS->xe$vms_code, IOS->xe$tran_size);
        return 0;
    };
...
// Issue the startup command to controller
    ...
    if (!((RC == SS$_NORMAL) && (IOS->xe$vms_code == SS$_NORMAL)))
    {
        ...
        XE$ERR(XE_Int, "XE ARP startup failure, RC=!XL,VMS_code=!XL,Xfer size=!SL", RC, IOS->xe$vms_code, IOS->xe$tran_size);
        return 0;
    };
...
XE_SenseDev(struct XE_Interface_Structure * XE_Int, struct xe_addrs_structure * phaddr, struct xe_Addrs_structure * hwaddr,
        long * online)
//
...
// Get info from controller
    ...
    if (!((RC == SS$_NORMAL) && (IOS->xe$vms_code)))
    {
        ...
        XE$ERR(XE_Int, "XE sense mode QIOW failure, RC=!XL,VMS_Code=!XL,Xfer size=!SL", RC, IOS->xe$vms_code, IOS->xe$tran_size);
        return 0;
    };
    ...
    while (Paramdescr->xe$setup_length > 0)
    {
        ...
        Paramdescr->xe$setup_length = Paramdescr->xe$setup_length - XE_Param_Size(Param);
        Paramdescr->xe$setup_address = Paramdescr->xe$setup_address + XE_Param_Size(Param);

        switch (Param->XE_Sense_Param)
        {
        case NMA$C_PCLI_PHA:
            CH$MOVE(XE_ADR_SIZE, CH$PTR(Param->XE_Sense_String), CH$PTR(phaddr));
            break;

        case NMA$C_PCLI_HWA:
            CH$MOVE(XE_ADR_SIZE, CH$PTR(Param->XE_Sense_String), CH$PTR(hwaddr));
            break;
        }
    };
...
// Check against address specified in CONFIG - report if different
        ...
        XE_ADR_SIZE, CH$PTR(addrs)))
        {
            ...
            XE_ADR_SIZE, l/*CH$PTR(UPLIT(0,0))*/))
            {
                ...
                ASCII_HEX_BYTES(newstr, XE_ADR_SIZE, useaddr, &newstr->dsc$w_length);
...
XE$CHECK(Device_Configuration_Entry * dev_config)
{
    ...
    {
        if ((!XE_Int->XEI$need_2_free) && xe_startall(XE_Int, TRUE))
        {
            // Device restarted OK
            XE_Int->XEI$restart_time = 0;
            XE_Int->XEI$retry_count = 0;
            XE_Int->XEI$restart_count = XE_Int->XEI$restart_count + 1;
            DRV$OPR_FAO("XE (DEC ENET) restarted, count = !SL", XE_Int->XEI$restart_count);
...
void XE$init(Device_Configuration_Entry * dev_config, long IPACP_Int, long max_retry, long MPBS)
{
    extern xearp$init();
    extern LIB$GET_VM();
    extern LIB$GET_VM_PAGE();
    signed long RC, rcvhdrs, XE_Chan, XAR_Chan;
    struct XE_Interface_Structure * XE_Int;
...
// Assign Ethernet Controller
    if (!((RC = sys$assign(&dev_config->dc_devname, &XE_Chan, 0, 0, 0)) & 1))
    // Ethernet controller assign failed
    {
        DRV$FATAL_FAO("XE $ASSIGN failure (dev=), EC = !XL", dev_config->dc_devname, RC);
        return;
    };

//  Assign the channel for the arp responder
    if (!((RC = sys$assign(&dev_config->dc_devname, &XAR_Chan, 0, 0, 0)) & 1))
    // Ethernet controller assign failed
    {
        DRV$FATAL_FAO("XE $ASSIGN failure (dev=), EC = !XL", dev_config->dc_devname, RC);
        return;
...
//    if (! (LIB$GET_VM(%REF(XE_Interface_size*4),XE_Int)))
    if (!(LIB$GET_VM_PAGE(/*%REF*/(((XE_Interface_size) / 512) + 1), &XE_Int)))
    {
        ...
        DRV$FATAL_FAO("XE LIB$GET_VM failure (dev=), EC = !XL", dev_config->dc_devname, RC);
        return;
    };
...
//    if (! (LIB$GET_VM(%REF(MAX_RCV_BUF*XERCV_LEN*4),rcvhdrs)))
    if (!(LIB$GET_VM_PAGE(/*%REF*/(((MAX_RCV_BUF * XERCV_LEN) / 512) + 1), &rcvhdrs)))
    {
        ...
        DRV$FATAL_FAO("XE LIB$GET_VM failure (dev=!AS), RC=!XL", dev_config->dc_devname, RC);
        return;
    };
...
void xe$xmit(Device_Configuration_Entry * dev_config)
{
    ...
    signed long RC, Sen_size, ARstat, AddrCheck, xchan;
...
// IPH$TTL in the swapped header is really IPH$Protocol
            if ((IPHDR->iph$protocol == UDP_PROTOCOL)
                    && (IPHDR->iph$dest == ((!dev_config->dc_ip_netmask) | dev_config->dc_ip_network)))
            {
...
//!!HACK!!// What's the EFN for?
                ...
                RC = sys$qiow(1, xchan,
                IO$_WRITEVBLK, IOS, 0, 0, QB->NSQ$Data, Sen_size, 0, 0, Sbuf, 0);
...
// Check for device driver error
                ...
                {
                    XE$ERR(XE_Int, "XE driver error (send),VMS_code=!XL", IOS->xe$vms_code);
                    goto leave_x;
                };
...
// Check for controller error
                ...
                {
                    DRV$XLOG_FAO(LOG$PHY, "%T XE command error !XW!/", 0, IOS->XE$cmd_status);
                    goto leave_x;
                };
            };
        };  // End of block X:
...
void XE_receive(struct XE_Interface_Structure * XE_Int)
{
    ...
    Device_Configuration_Entry * dev_config;
    signed long rcvix, RC, IRC, Error_Flag = 0;
...
//  Error_Flag = 1;
        ...
        {
        case SS$_ABORT:
            DRV$OPR_FAO("XE abort, cmd_status=!XW, error_summary=!XB", Rbuf->XERCV$cmd_status, Rbuf->XERCV$error_summary);
            break;
        ...
        default:
            DRV$OPR_FAO("XE: VMS Error, cmd_status=!XW, error_summary=!XB", Rbuf->XERCV$cmd_status, Rbuf->XERCV$error_summary);
        }
    };
    ...
    if ((Rbuf->XERCV$cmd_status & 0x0FF00) != XM$M_STS_ACTIVE)
    {
        Error_Flag = 1;
        // Error from board
        DRV$OPR_FAO("XE status error.  Status = !XL", Rbuf->XERCV$cmd_status & 0x0FF00);
    };
...
// Get another buffer and put it on the receive Q for this device
    ...
    IO$_READVBLK, &NRbuf->XERCV$vms_code, XE_receive, XE_Int, &NRbuf->XERCV$data,
    DRV$MAX_PHYSICAL_BUFSIZE, 0, 0, rcvhdrs[XE_Int->XEI$curhdr].XERCV$buf, 0);
...
// Send datagram to IP
        ...
        dev_config->dcmib_ifInOctets = dev_config->dcmib_ifInOctets + Rbuf->XERCV$tran_size + XE_hdr_len;

        drv$ip_receive(Rbuf-XE_hdr_offset, DRV$MAX_PHYSICAL_BUFSIZE+(Qhead_len+IOS_len), &Rbuf->XERCV$data, Rbuf->XERCV$tran_size,
                dev_config);
    };
...
void xe_arprcv(struct XE_Interface_Structure * XE_Int)
{
    ...
    struct xe_arp_structure * ARbuf;
    signed long usflag, tpa, I, spa, Route, RC;
...
// check status of receive
    ...
    if (((ARbuf->ar_ios0 != SS$_NORMAL) && (ARbuf->ar_ios0 != SS$_DATAOVERUN)))
        // Error from DEC driver
        if (ARbuf->ar_ios0 == SS$_ABORT)
            DRV$OPR_FAO("XE abort, cmd_status=!XW, error_summary=!XB", ARbuf->ar_ios2, ARbuf->ar_ios3);
        else
...
// Write the ARP to the network (don't need to build any header info)
    ...
    rc = sys$qiow(0, XE_Int->xei$arp_io_chan, IO$_WRITEVBLK, ios, 0, 0, arbuf, arlen, 0, 0, dest, 0);
    if (rc != SS$_NORMAL)
    {
