...
// TCP connection table
    ...
    "MIB, 6, 13, 1, 1, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF"),
    $MIBent( var_tcpConn, 20, IPADDRESS, TCPCONNLOCALADDRESS, RONLY,
    "MIB, 6, 13, 1, 2, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF"),
    $MIBent( var_tcpConn, 20, INTEGER, TCPCONNLOCALPORT, RONLY,
    "MIB, 6, 13, 1, 3, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF"),
    $MIBent( var_tcpConn, 20, IPADDRESS, TCPCONNREMADDRESS, RONLY,
    "MIB, 6, 13, 1, 4, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF"),
    $MIBent( var_tcpConn, 20, INTEGER, TCPCONNREMPORT, RONLY,
    "MIB, 6, 13, 1, 5, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF"),
    ...
    * vtcb_ptr,
    ...
    for (j=1; j<=vtcb_size; j++)
        if ((tcb = vtcb_ptr[j]) != 0)
        {
            ...
            newname[10] = tcpaddr[0];
            newname[11] = tcpaddr[1];
            newname[12] = tcpaddr[2];
            newname[13] = tcpaddr[3];
            newname[14] = tcb->local_port;  // Local Port

            tcpaddr = &tcb->foreign_host;   // Remote Host
            newname[15] = tcpaddr[0];
            newname[16] = tcpaddr[1];
            newname[17] = tcpaddr[2];
            newname[18] = tcpaddr[3];
