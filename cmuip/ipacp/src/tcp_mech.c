...
extern window_default;

struct _generic_64 Start_Time;   // Quadword time IPACP started.
struct _generic_64 TEK$sys_uptime;   // Quadword delta time since IPACP started.
...
signed long max_local_ports = 30;  // Settable in CONFIG
...
long * vtcb_ptr;
signed long max_tcb = 0, max_conn = 60,  // Settable in CONFIG
        vtcb_size = 0, tcb_count = 0;  // # of valid TCB's in VTCB table.
...
{
    extern MOVBYT(), mm$get_mem(), mm$free_mem();
    signed long J, Old, idx;
...
// which ever is approprate.

                {
                    ...
                    while ((OTCB != &ConectPtr[CN_Index].CN$TCB_List) && (!ok))
                    {
                        if ((OTCB->foreign_host == WILD) || (OTCB->foreign_port == WILD))
                        {
                            ok = TRUE;
    ...
    signed long * IDX;
{
    ...
    signed long Index, LP, FP, Unique;
...
// Compare rtn call arg connection(Fhost,Fport) with TCB(Foreign_Host&Port).
        ...
        *IDX = Index;       // return conect index.
        ...
        {
            if ((TCB->foreign_host == Fhost) && ((TCB->foreign_port & 0xFFFF) == FP) && (TCB->state != CS$INACTIVE))
                Unique = FALSE;
            else
                TCB = TCB->lp_next;
        }
...
tcb$create(void)
{
    ...
    signed long Indx,   // location of new TCB in Valid TCB table.
...
// functions), neither of these checks should ever fail.
    ...
    if ((TCB->vtcb_index != TCBIDX) || (TCB->ucb_adrs != uargs->ud$ucb_adrs))
...
// in the global variable TEK$sys_uptime, a quadword.  Also returns the
{
    ...
    struct _generic_64 One;  // QuadWord of val 1.
    struct _generic_64 Now;    // time as in now.

    One.gen64$r_quad_overlay.gen64$q_quadword = 1;
...
// time in quadwords.
    ...
    Now.gen64$r_quad_overlay.gen64$q_quadword = -Now.gen64$r_quad_overlay.gen64$q_quadword;    // Compute Delta time (quadword).
    Addm(2, Now, One, TEK$sys_uptime); // compute Delta TCP uptime.

    sys$numtim(time_buffer, &TEK$sys_uptime);

    // time in hundredth of seconds
    uptime = time_buffer[6] + time_buffer[5] * 100 + time_buffer[4] * 100 * 60 + time_buffer[3] * 100 * 60 * 60
            + time_buffer[2] * 100 * 60 * 60 * 24 + time_buffer[1] * 100 * 60 * 60 * 24 * 30;
}
