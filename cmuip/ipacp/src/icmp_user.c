...
extern signed long intdf, ast_in_progress, log_state, min_physical_bufsize, max_physical_bufsize;
...
void Deliver_ICMP_Data(struct ICMPCB_Structure * ICMPCB, struct queue_blk_structure(qb_nr_fields) * QB,
...
signed long icmpttl = 32;  // TTL for ICMP

static signed long ICMPIPID = 1,  // Current IP packet ID
...
// Log the contents of the ICMP header
    ...
    QL$FAO("!%T !AS ICMP packet, SEG=!XL, DATA=!XL!/!_CKsum:!_!SL!/", 0, &sptr, Seg, segdata, seghdr->icm$cksum);
...
ICMPCB_Find( Src$Adrs)
{
    ...
    while ((Ucount > 0) && (ICMPCBIX <= MAX_ICMPCB))
    {
        ...
        {
            if (((ICMPCB->ICMPCB$Foreign_Host == WILD) || (ICMPCB->ICMPCB$Foreign_Host == Src$Adrs)))
                return ICMPCB;
            Ucount = Ucount - 1;
        };
        ICMPCBIX = ICMPCBIX + 1;
...
void icmp$user_input(Src$Adrs, Dest$Adrs, bufsize, Buf, SegSize, Seg)
    struct icmp_header * Seg;
{
    signed long Buf2, Ucount, ICMPCBIX, sum, delete;
    struct ICMPCB_Structure * ICMPCB;
...
// Log that it was found
            ...
            if ($$LOGF(LOG$ICMP))
                QL$FAO("!%T ICMPCB !XL found for ICMP Seg !XL!/", 0, ICMPCB, Seg);

// Make sure the ICMPCB isn't aborted...

            if (ICMPCB->ICMPCB$Aborting)
            {
                XQL$FAO(LOG$ICMP, "!%T ICMP input !XL for aborted ICMPCB !XL dropped!/", 0, Seg, ICMPCB);
                goto leave_x;
            };
...
Queue_User_ICMP(ICMPCB, Uptr, usize, Buf, bufsize, QB)
    struct ICMPCB_Structure * ICMPCB;struct queue_blk_structure(qb_nr_fields) * QB;
{
    signed long QBR;
    extern mm$qblk_get();
...
void Deliver_ICMP_Data(ICMPCB, QB, URQ)
    struct ICMPCB_Structure * ICMPCB;struct queue_blk_structure(qb_nr_fields) * QB;struct queue_blk_structure(qb_ur_fields) * URQ;
{
    signed long FLAGS, ICMTYPE, Aptr, Uptr, Ucount;
    struct _irp * IRP;
...
// Truncate to user receive request size
    ...
    if ($$LOGF(LOG$ICMP))
        QL$FAO("!%T Posting ICMP receive,Size=!SL,ICMPCB=!XL,IRP=!XL,UCB_A=!XL!/", 0, Ucount, ICMPCB, URQ->ur$irp_adrs,
                URQ->ur$ucb_adrs);
...
// block from our local copy of uargs.
    ...
    if (uargs->re$ph_buff != 0)
        $$KCALL(MOVBYT, IPADR$ADDRESS_BLEN, Aptr, uargs->re$ph_buff);

// Post the I/O and free up memory

    user$post_io_status(URQ->ur$uargs, SS$_NORMAL, Ucount, 0, 0);
...
// Check consistancy of ICMPCB back-pointer into table
    ...
    if ((ICMPCB->icmpcb$icmpcbid != Conn_ID) || (ICMPCB->ICMPCB$UCB_ADRS != uargs->ud$ucb_adrs))
    ...
    long * IDX;
{
    ...
    struct ICMPCB_Structure * ICMPCB;
    signed long I, ICMPCBIDX, RC, Pages;
...
{
    signed long IPADDR, NAMLEN, NAMPTR, UIDX, icmpcbptr, Args[4];
    struct ICMPCB_Structure * ICMPCB;

    XLOG$FAO(LOG$USER, "!%T ICMP$OPEN: PID=!XL,CHAN=!XW,FLAGS=!XL X1=!XL!/", 0, uargs->op$pid, uargs->op$piochan, uargs->op$flags,
            uargs->op$ext1);
...
// Set local and foreign host numbers according to our info
    ...
    if (ADRCNT > 0)
        IP$SET_HOSTS(ADRCNT, ADRLST, &ICMPCB->ICMPCB$Local_Host, &ICMPCB->ICMPCB$Foreign_Host);

// Done at last - log success

    XLOG$FAO(LOG$USER, "!%T UDB_COPEN: Conn idx = !XL, ICMPCB = !XL!/", 0, ICMPCB->icmpcb$icmpcbid, ICMPCB);
    return SS$_NORMAL;
}
...
{
    struct icmp_header * Seg;
    signed long RC, bufsize, Buf, Segsize, usize;
...
// Validate connection ID and get ICMPCB pointer
    ...
    if ((ICMPCB = ICMPCB_OK(uargs->se$local_conn_id, &RC, uargs)) == 0)
    {
        ...
        icmp_mib->MIB$icmpOutErrors = icmp_mib->MIB$icmpOutErrors + 1;
        return;
    };
    XLOG$FAO(LOG$USER, "!%T ICMP$SEND: Conn=!XL, ICMPCB=!XL, Size=!SL!/", 0, uargs->se$local_conn_id, ICMPCB, uargs->se$buf_size);
...
// Send the segment to IP (it will deallocate it)
    ...
    RC = SS$_NORMAL;
    if ((ip$send(LocalAddr, ForeignAddr, ICMPTOS, icmpttl, Seg, Segsize, ICMPIPID, ICMPDF, TRUE, ICMP_Protocol, Buf, bufsize) == 0))
        RC = NET$_NRT;
    ...
    {
    case ICM_ECHO:
        icmp_mib->MIB$icmpOutEchos = icmp_mib->MIB$icmpOutEchos + 1;
        break;
    case ICM_TSTAMP:
        icmp_mib->MIB$icmpOutTimeStamps = icmp_mib->MIB$icmpOutTimeStamps + 1;
        break;
    case ICM_AMREQUEST:
        icmp_mib->MIB$icmpOutAddrMasks = icmp_mib->MIB$icmpOutAddrMasks + 1;
        break;
    case ICM_DUNREACH:
        icmp_mib->MIB$icmpOutDestUnreachs = icmp_mib->MIB$icmpOutDestUnreachs + 1;
        break;
    case ICM_SQUENCH:
        icmp_mib->MIB$icmpOutSrcQuenchs = icmp_mib->MIB$icmpOutSrcQuenchs + 1;
        break;
    case ICM_REDIRECT:
        icmp_mib->MIB$icmpOutRedirects = icmp_mib->MIB$icmpOutRedirects + 1;
        break;
    case ICM_TEXCEED:
        icmp_mib->MIB$icmpOutTimeExcds = icmp_mib->MIB$icmpOutTimeExcds + 1;
        break;
    case ICM_PPROBLEM:
        icmp_mib->MIB$icmpOutParamProbs = icmp_mib->MIB$icmpOutParamProbs + 1;
        break;
    case ICM_TSREPLY:
        icmp_mib->MIB$icmpOutTimeStampReps = icmp_mib->MIB$icmpOutTimeStampReps + 1;
        break;
    case ICM_AMREPLY:
        icmp_mib->MIB$icmpOutAddrMaskReps = icmp_mib->MIB$icmpOutAddrMaskReps + 1;
        break;
    case ICM_EREPLY:
        icmp_mib->MIB$icmpOutEchoReps = icmp_mib->MIB$icmpOutEchoReps + 1;
        break;
    };
...
// Validate connection ID and get ICMPCB pointer
    ...
    XLOG$FAO(LOG$USER, "!%T ICMP$RECEIVE: Conn=!XL, ICMPCB=!XL, Size=!SL!/", 0, uargs->re$local_conn_id, ICMPCB,
            uargs->re$buf_size);
...
// Give the information back (common TCP/ICMP routine in USER.BLI)
    ...
    user$net_connection_info(uargs, ICMPCB->ICMPCB$Local_Host, ICMPCB->ICMPCB$Foreign_Host, 0, 0, ICMPCB->ICMPCB$Foreign_Hname,
...
// If the process doing the cancel owns this connection, then delete it.
            ...
            if ((ICMPCB->ICMPCB$User_ID == uargs->vc$pid) && (ICMPCB->ICMPCB$PIOchan == uargs->vc$piochan))
            {
...
// Validate that there is a real ICMPCB there
    ...
    if ((ICMPCBIX < 1) || (ICMPCBIX > MAX_ICMPCB) || ((ICMPCB = icmpcb_table[ICMPCBIX]) == 0))
        return FALSE;
