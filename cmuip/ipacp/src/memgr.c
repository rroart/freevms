...
unsigned char qblk_count_base = 0,   // #Qblks on free list.
...
signed long qb_gets = 0,   // Queue Blks dynamically allocated
...
//    if (! (RC = LIB$GET_VM(Size,Addr)))
    if (!((RC = LIB$GET_VM_PAGE(Pages, Addr)) & 1))
...
void mm$free_mem( Mem, Size)
{
    ...
    NOINT
    ;          // Hold AST's
    if (!((RC = LIB$FREE_VM_PAGE(Pages, Mem)) & 1))
        Memgr_Fault_Handler(0, RC, 0);
    OKINT
    ;
}
...
//  LIB$GET_VM(%REF((qb_size+MEM$HDR_SIZE)*4),Hptr);
        Pages = ((((qb_size + MEM$HDR_SIZE)) / 512) + 1);
        if (!((RC = (LIB$GET_VM_PAGE(Pages, &Hptr))) & 1))
            Memgr_Fault_Handler(0, RC, 0);
...
mm$uarg_get(void)
{
    ...
    {
        NOINT
        ;          // Disable interrupts, do allocation
...
//  LIB$GET_VM(%REF(Max_User_ArgBlk_Size*4),Ptr);
        Pages = (((Max_User_ArgBlk_Size) / 512) + 1);
        if (!((RC = (LIB$GET_VM_PAGE(Pages, &Ptr))) & 1))
            Memgr_Fault_Handler(0, RC, 0);
...
long mm$seg_get( Size)
{
    ...
    R0;// standard vax/vms routine return value register.
#endif
    signed long Alloc = FALSE,  // no seg allocation yet.
            Ptr, Pages;
...
void mm$seg_free( Size, Ptr)
{
    ...
    R0; // standard vax/vms routine return value register.
#endif
    signed long Released = FALSE, Pages;
