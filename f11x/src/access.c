...
int ods2_block_read_full_page3(struct _wcb * wcb, struct page *page,
                               unsigned long pageno)
{
    ...
    do
    {
        ...
        if (vcb->vcb$l_rvn)
        {
            ...
            ucb = vcb->vcb$l_rvt;
        sts = exe$qiow(XQP_EF, (unsigned short) xqp->io_channel, IO$_READPBLK,
                       &iosb, 0, 0, page_address(page) + i * blocksize, blocksize,
                       blocknr, ucb->ucb$w_fill_0, 0, 0);
...
static int __ods2_block_write_full_page2(struct _wcb *wcb, struct page *page,
        unsigned long pageno)
{
    struct _fcb * fcb = wcb->wcb$l_fcb;
    struct _vcb * vcb = xqp->current_vcb;
    ...
    do
    {
        ...
        if (vcb->vcb$l_rvn)
        {
            ...
            ucb = vcb->vcb$l_rvt;
        sts = exe$qiow(XQP_EF, (unsigned short) xqp->io_channel, IO$_WRITEPBLK,
                       &iosb, 0, 0, page_address(page) + turns * blocksize, blocksize,
                       blocknr, ucb->ucb$w_fill_0, 0, 0);
...
int ods2_block_write_full_page3(struct _wcb * wcb, struct page *page,
                                unsigned long pageno)
{
    struct _fcb * fcb = wcb->wcb$l_fcb;
                       ...
                       unsigned long count, struct _iosb * iosb)
{
    ...
    if (vcb->vcb$l_rvn)
    {
        ...
        ucb = vcb->vcb$l_rvt;
    unsigned long sts = sys$qiow(XQP_EF, xqp->io_channel, IO$_READLBLK, &myiosb,
                                 0, 0, buf, 512 * count, phyblk, ucb->ucb$w_fill_0, 0, 0);
    if (iosb)
                        ...
                        unsigned long lbn, unsigned long count, struct _iosb * iosb)
{
    ...
    unsigned long sts = sys$qiow(XQP_EF, xqp->io_channel, IO$_WRITELBLK,
                                 &myiosb, 0, 0, buf2, 512 * count, phyblk, ucb->ucb$w_fill_0, 0, 0);
    kfree(buf2);
#else
...
void * f11b_search_fcb(struct _vcb * vcb, struct _fiddef * fid)
{
    ...
    while (tmp != head)
    {
        if ((tmp->fcb$b_fid_nmx == fid->fid$b_nmx)
                && (tmp->fcb$w_fid_num == fid->fid$w_num))
            return tmp;
        tmp = tmp->fcb$l_fcbfl;
    }
    return 0;
    ...
    while (atrp->atr$w_type != 0)
    {
        ...
        case ATR$C_CREDATE:
        {
            if (head->fh2$w_struclev == 0x501)
                memcpy(atrp->atr$l_addr, &((struct _fi5 *) fi)->fi5$q_credate,
                       atrp->atr$w_size);
            else
                memcpy(atrp->atr$l_addr, &((struct _fi2 *) fi)->fi2$q_credate,
                       atrp->atr$w_size);
        }
        break;
        case ATR$C_REVDATE:
        {
            if (head->fh2$w_struclev == 0x501)
                memcpy(atrp->atr$l_addr, &((struct _fi5 *) fi)->fi5$q_revdate,
                       atrp->atr$w_size);
            else
                memcpy(atrp->atr$l_addr, &((struct _fi2 *) fi)->fi2$q_revdate,
                       atrp->atr$w_size);
        }
        break;
        case ATR$C_EXPDATE:
        {
            if (head->fh2$w_struclev == 0x501)
                memcpy(atrp->atr$l_addr, &((struct _fi5 *) fi)->fi5$q_expdate,
                       atrp->atr$w_size);
            else
                memcpy(atrp->atr$l_addr, &((struct _fi2 *) fi)->fi2$q_expdate,
                       atrp->atr$w_size);
        }
        break;
        case ATR$C_BAKDATE:
        {
            if (head->fh2$w_struclev == 0x501)
                memcpy(atrp->atr$l_addr, &((struct _fi5 *) fi)->fi5$q_bakdate,
                       atrp->atr$w_size);
            else
                memcpy(atrp->atr$l_addr, &((struct _fi2 *) fi)->fi2$q_bakdate,
                       atrp->atr$w_size);
        }
        break;
...
void f11b_write_attrib(struct _fcb * fcb, struct _atrdef * atrp)
{
    ...
    while (atrp->atr$w_type != 0)
    {
        ...
        {
            if (head->fh2$w_struclev == 0x501)
                memcpy(&((struct _fi5 *) fi)->fi5$q_credate, atrp->atr$l_addr,
                       atrp->atr$w_size);
            else
                memcpy(&((struct _fi2 *) fi)->fi2$q_credate, atrp->atr$l_addr,
                       atrp->atr$w_size);
        }
        break;
        case ATR$C_REVDATE:
        {
            if (head->fh2$w_struclev == 0x501)
                memcpy(&((struct _fi5 *) fi)->fi5$q_revdate, atrp->atr$l_addr,
                       atrp->atr$w_size);
            else
                memcpy(&((struct _fi2 *) fi)->fi2$q_revdate, atrp->atr$l_addr,
                       atrp->atr$w_size);
        }
        break;
        case ATR$C_EXPDATE:
        {
            if (head->fh2$w_struclev == 0x501)
                memcpy(&((struct _fi5 *) fi)->fi5$q_expdate, atrp->atr$l_addr,
                       atrp->atr$w_size);
            else
                memcpy(&((struct _fi2 *) fi)->fi2$q_expdate, atrp->atr$l_addr,
                       atrp->atr$w_size);
        }
        break;
        case ATR$C_BAKDATE:
        {
            if (head->fh2$w_struclev == 0x501)
                memcpy(&((struct _fi5 *) fi)->fi5$q_bakdate, atrp->atr$l_addr,
                       atrp->atr$w_size);
            else
                memcpy(&((struct _fi2 *) fi)->fi2$q_bakdate, atrp->atr$l_addr,
                       atrp->atr$w_size);
        }
        break;
...
struct _fcb * getidxfcb(struct _vcb * vcb)
{
    ...
    while (tmp != head)
    {
        if (tmp->fcb$w_fid[0] == 1 && tmp->fcb$w_fid[0] == 1)
            return tmp;
        tmp = tmp->fcb$l_fcbfl;
    }
    return 0;
...
struct _fcb * getmapfcb(struct _vcb * vcb)
{
    ...
    while (tmp != head)
    {
        if (tmp->fcb$w_fid[0] == 2 && tmp->fcb$w_fid[0] == 2)
            return tmp;
        tmp = tmp->fcb$l_fcbfl;
    }
    return 0;
...
unsigned writechunk(struct _fcb * fcb, unsigned long vblock, char * buff)
{
    ...
    sts = sys$qiow(XQP_EF, xqp->io_channel, IO$_WRITELBLK, &iosb, 0, 0, buff,
...
int get_fm2_val(unsigned short ** mpp, unsigned int * phyblk,
                unsigned int *phylen)
{
...
void *f11b_readvblock(struct _fcb * fcb, unsigned curvbn, unsigned *retsts)
{
    ...
    do
    {
        ...
        else
        {
            ...
            unsigned phyblk, phylen;
            struct _vcb *vcbdev;
            sts = getwindow(fcb, curvbn, &vcbdev, &phyblk, &phylen, NULL,
                            NULL );
            if (sts & 1)
            {
                if (phylen > length)
               ...
               struct _vcb **retvcb)
{
    ...
    int islocal;
    int isfile = 0;
    if (sizeof(struct _hm2) != 512 || sizeof(struct _fh2) != 512)
    ...
    for (device = 0; device < devices; device++)
    {
        ...
        if (devices > 1)
        {
            vcb->vcb$l_rvt = kmalloc(
                                 sizeof(struct _rvt) + (devices - 1) * sizeof(long),
                                 GFP_KERNEL);
            memset(vcb->vcb$l_rvt, 0,
                   sizeof(struct _rvt) + (devices - 1) * sizeof(long));
        }
        ucb->ucb$l_vcb = vcb;
        sts = SS$_NOSUCHVOL;
        ...
        if (strlen(devnam[device]))
        {
            ...
            {
#ifdef __x86_64__
                sts = sys$qiow(XQP_EF, chan, IO$_READLBLK, &iosb, 0, 0,
                               (char *) home2, sizeof(struct _hm2), hba,
        ...
        if (sts & 1)
        {
            ...
            if (strlen(devnam[device]))
            {
                ...
                else
                {
                    ...
                    if (1)
                    {
                        struct _fibdef mapfib =
                            { 0, 2, 2, 1, 0 };
                        struct dsc$descriptor mapdsc;
                        struct _irp * dummyirp = kmalloc(sizeof(struct _irp),
                                                         GFP_KERNEL);
                        ...
                        if (sts & 1)
                        {
                            ...
                            sts = ioc_std$mapvblk(1, 0, &mapfcb->fcb$l_wlfl, 0,
                                                  0, &vcb->vcb$l_sbmaplbn, 0, 0);
                            sts = accesschunk(mapfcb, 1, (char **) &scb, NULL,
                                              0, 0);
                            if (sts & 1)
                            {
                                vcbdev->vcb$l_sbmaplbn =
                                    mapfcb->fcb$l_wlfl->wcb$l_p1_lbn;
                                vcbdev->vcb$l_sbmapsize = scb->scb$l_volsize
                                                          / (512 * 8 * scb->scb$w_cluster) + 1;
                                if (scb->scb$w_cluster == home.hm2$w_cluster)
                                {
                                    //vcbdev->max_cluster = (scb->scb$l_volsize + scb->scb$w_cluster - 1) / scb->scb$w_cluster;
                                    sts = update_freecount(vcbdev,
                                                           &vcbdev->vcb$l_free);
