...
{
    printk(
        "DIRECT_SHOW Lookups: %d Searches: %d Deletes: %d Inserts: %d Splits: %d\n",
        direct_lookups, direct_searches, direct_deletes, direct_inserts,
        direct_splits);
}
...
unsigned insert_ent(struct _fcb * fcb, unsigned eofblk, unsigned curblk,
                    char *buffer, struct _dir * dr, struct _dir1 * de, char *filename,
                    unsigned filelen, unsigned version, struct _fiddef * fid)
{
    struct _iosb iosb;
    unsigned sts = 1;
    int inuse = 0;
...

    {
        ...
        if (de != NULL )
        {
            if ((dr->dir$w_size > MAXREC)
                    || (char *) de < dr->dir$t_name + dr->dir$b_namecount
                    || (char *) de > (char *) dr + dr->dir$w_size + 2)
            {
    ...
    if (addlen > MAXREC - inuse)
    {
        ...
        eofblk++;
        head = f11b_read_header(fcb->fcb$l_wlfl->wcb$l_orgucb->ucb$l_vcb, 0,
                                fcb, &iosb);
        sts = iosb.iosb$w_status;
        head->fh2$w_recattr.fat$l_efblk = eofblk + 1;

        /* First find where the next record is... */

        nr = dr;
        if (nr->dir$w_size <= MAXREC)
            nr = (struct _dir *) ((char *) nr + nr->dir$w_size + 2);

        /* Can we split between records? */

        if (de == NULL
                || (char *) dr != buffer|| (nr->dir$w_size <= MAXREC))
        {
            struct _dir *sp = dr;
        ...
        {
            unsigned reclen = (dr->dir$b_namecount + STRUCT_DIR_SIZE) & ~1;
            ...
            nbr->dir$w_size = reclen + ((char *) nr - (char *) de) - 2;

            memset((char *) de + 2, 0, ((char *) nr - (char *) de));
            ((struct _dir *) de)->dir$w_size = 0xffff;
            dr->dir$w_size = ((char *) de - (char *) dr) - 2;
            if ((char *) de >= (char *) nr)
            {
    ...
    {
        dr->dir$w_size = dr->dir$w_size + addlen;
...
unsigned delete_ent(struct _fcb * fcb, unsigned curblk, struct _dir * dr,
                    struct _dir1 * de, char *buffer, unsigned eofblk)
{
    struct _iosb iosb;
    unsigned sts = 1;
    unsigned ent;
    struct _fh2 * head;
    direct_deletes++;
    ent = (dr->dir$w_size - STRUCT_DIR_SIZE - dr->dir$b_namecount + 3)
          / sizeof(struct _dir1);
    if (ent > 1)
    {
        char *ne = (char *) de + sizeof(struct _dir1);
        memcpy(de, ne, BLOCKSIZE - (ne - buffer));
        dr->dir$w_size = dr->dir$w_size - sizeof(struct _dir1);
    }
    else
    {
        char *nr = (char *) dr + dr->dir$w_size + 2;
...
unsigned return_ent(struct _fcb * fcb, unsigned curblk, struct _dir * dr,
                    struct _dir1 * de, struct _fibdef * fib, unsigned short *reslen,
                    struct dsc$descriptor * resdsc, int wildcard)
{
    struct _iosb iosb;
    int scale = 10;
    int version = de->dir$w_version;
                    ...
                    struct dsc$descriptor * resdsc, unsigned eofblk, unsigned action)
{
    ...
    else
    {
        ...
        sts = name_check(searchspec, filedsc->dsc$w_length, &searchlen,
                         &version, &wildcard);
...

    {
        ...
        while ((sts & 1) && curblk <= eofblk)
        {
            ...
            {
                if (name_match(last_name, last_len, dr->dir$t_name,
                               last_len) != MAT_EQ)
                {
                    relver = 0;
                }
            ...
            do
            {
                ...
                if (cmp == MAT_GT && wcc_flag)
                {
                    wcc_flag = 0;
                    searchspec = filedsc->dsc$a_pointer;
                    sts = name_check(searchspec, filedsc->dsc$w_length,
                ...
                else
                {
                    if (cmp == MAT_EQ)
                    {
                        ...
                        while ((char *) de < nr)
                        {
                            ...
                            {
                                if (version
                                        > 32767|| version == relver || version == de->dir$w_version)
                                {
                                    cmp = MAT_EQ;
                                }
                                else
                                ...
                                else
                                {
                                    ...
                                    sts = name_check(searchspec,
                                                     filedsc->dsc$w_length, &searchlen,
                                                     &version, &wildcard);
                                    if ((sts & 1) == 0)
                                        break;
                        ...
                        if (cmp == MAT_EQ)
                        {
                            ...
                            case 0:
                            {
                                int ret = return_ent(fcb, curblk, dr, de, fib,
                            ...
                            case 1:
                            {
                                int ret = delete_ent(fcb, curblk, dr, de,
                                                     buffer, eofblk);
                                kfree(buffer);
                                return ret;
                            }
                        ...
                        else
                        {
                            ...
                            {
                                return insert_ent(fcb, eofblk, curblk, buffer,
                                                  dr, de, searchspec, searchlen, version,
                    ...
                    if (((struct _dir *) nr)->dir$w_size > MAXREC)
                    {
