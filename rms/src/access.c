...
{
    register struct WCB *wcb = (struct WCB *) vmalloc(sizeof(struct WCB));
    if (wcb == NULL) {
        *retsts = SS$_INSFMEM;
    } else {
        unsigned curvbn;
        ...
        if (wcbkey->prevwcb == NULL) {
            ...
            memcpy(&wcb->hd_fid,&wcbkey->prevwcb->hd_fid,sizeof(struct _fiddef));
        }
        do {
        ...
        if (vioc != NULL) {
            ...
            if (wcbkey->fcb->head == NULL) vfree(head);
        }
        wcb->hiblk = curvbn - 1;
                   ...
                   unsigned *hdrseq)
{
    ...
    if (wcb == NULL) return sts;
    {
        ...
        *phylen = wcb->phylen[extent] - togo;
        if (hdrfid != NULL) memcpy(hdrfid,&wcb->hd_fid,sizeof(struct _fiddef));
...
{
    register struct VIOC *vioc = (struct VIOC *) vmalloc(sizeof(struct VIOC));
    if (vioc == NULL) {
        *retsts = SS$_INSFMEM;
    } else {
        register int length;
