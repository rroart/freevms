...
char scs_rt_all_end_mcast[ETH_ALEN] =
    { 0xAB, 0x00, 0x04, 0x01, 0x00, 0x00 };
char scs_rt_all_rt_mcast[ETH_ALEN] =
    { 0xAB, 0x00, 0x04, 0x01, 0x00, 0x00 };
char scs_hiord[ETH_ALEN] =
    { 0xAB, 0x00, 0x04, 0x01, 0x00, 0x00 }; // remember to add clusterid + 1
unsigned char scs_eco_version[3] =
    { 0x02, 0x00, 0x00 };
...
void scs_rt_finish_output2(char *buf, int len, char *dst);

#define DRV$MAX_PHYSICAL_BUFSIZE 1600
#define     MAX_RCV_BUF   64

struct scs_interface_structure scs_int_, *scs_int = &scs_int_;
...
void scs_dev_delete(struct net_device *dev);

void scs_send_brd_hello(struct net_device *dev);
...
void scs_rt_finish_output2(char * buf, int len, char *dst)
{
    ...
    IO$_WRITEVBLK, IOS, 0, 0, buf, len, 0, 0, dst, 0);
...
{
    return (msg->nisca$w_dx_src_hi == sr_hi) && (msg->nisca$l_dx_src_lo == sr_lo);
}

void scs_send_endnode_hello(struct net_device *dev)
{
    struct _nisca *intro;
...
void scs_dev_set_timer();

void scs_dev_timer_func(unsigned long arg)
{
    struct net_device *dev = (struct net_device *) arg;

    if (dev)
        scs_send_brd_hello(dev);
...
void scs_nsp_send_conninit2(struct _cdt *sk, unsigned char msgflg, char * rprnam, char * lprnam, char * condat)
{
    ...
    if (condat)
        memcpy(&scs->scs$b_con_dat, condat, strlen(condat));
...
void printscs(void * skbdata)
{
    ...
    printk("\nSCS\n");
    printk("dx %x %x %x %x %x\n", nisca->nisca$w_dx_group, nisca->nisca$w_dx_dst_hi, nisca->nisca$l_dx_dst_lo,
            nisca->nisca$w_dx_src_hi, nisca->nisca$l_dx_src_lo);
    printk("ni %x %x %x %x %x\n", nisca->nisca$b_msg, nisca->nisca$l_maint, nisca->nisca$b_tr_flag, nisca->nisca$b_tr_pad,
            nisca->nisca$b_tr_pad_data_len);
...
unsigned long nisca_dispatch[0x17] =
    { nisca_snt_dg, nisca_snt_lb, nisca_snt_seq, nisca_snt_datm, nisca_snt_dat, nisca_snt_dat_lp, nisca_req_id, nisca_req_datm,
        nisca_req_dat0, nisca_req_dat1, nisca_req_dat2, nisca_ret_datm, nisca_ret_dat, nisca_ret_dat_lp, nisca_ret_cnfm,
        nisca_ret_cnf, nisca_ret_id, nisca_ret_lb, nisca_reset, nisca_start, nisca_snt_datwm, nisca_snt_datwm_lp, nisca_ret_cnfwm };

unsigned long opc_dispatch[0x40] =
    { 0, opc_snddg, opc_sndmsg, opc_retcnf, 0, opc_reqid, opc_sndrst, opc_sndstrt, opc_reqdat, opc_reqdat1, opc_reqdat2, 0, 0,
        opc_sndlb, opc_reqmdat, 0, opc_snddat, opc_retdat, opc_sndmdat, 0, 0, 0, 0, 0, opc_invtc, opc_setckt, opc_rdcnt, 0,
        opc_setckt_len, 0, 0, 0, 0, opc_dgrec,
        opc_msgrec, // not used?
        opc_cnfrec, 0, 0, 0, 0, 0, opc_mcnfrec, 0, opc_idrec, 0, opc_lbrec, 0, 0, 0, opc_datrec, 0, opc_mdatrec, 0, 0, 0, 0, 0, 0,
        0, 0, opc_snddatwm, opc_cnfwmrec, opc_retcnfwm, 0 };
...
int scs_rcv(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt)
{
    ...
    dump_it: out: return NET_RX_DROP;
}
...
// Issue the startup command to controller
    ...
    if (!((RC == SS$_NORMAL) && (IOS->xe$vms_code == SS$_NORMAL)))
    {
        if (IOS->xe$vms_code == SS$_BADPARAM)
            printk("SCS startup failure, RC=%x,VMS_code=%x,Param=%x", RC, IOS->xe$vms_code, ((long *) IOS)[1]);
        else
            printk("SCS startup failure, RC=%x,VMS_code=%x,Xfer size=%x", RC, IOS->xe$vms_code, IOS->xe$tran_size);
        return 0;
    };
...
int scs_startio(int dummy)
{
    ...
    for (i = 0; i <= (MAX_RCV_BUF - 1); i++)
    {
        ...
        sts = sys$qio(2, scs_int->sei$io_chan, IO$_READVBLK, &buff->scs_rcv$vms_code, scs_receive, scs_int, &buff->scs_rcv$data,
...
void scs_receive(int i)
{
    ...
    IO$_READVBLK, &NRbuf->scs_rcv$vms_code, scs_receive, scs_int, &NRbuf->scs_rcv$data,
    DRV$MAX_PHYSICAL_BUFSIZE, 0, 0, rcvhdrs[scs_int->sei$curhdr].XERCV$buf, 0);
