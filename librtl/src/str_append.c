...
int str$append(struct dsc$descriptor_s* destination_string, const struct dsc$descriptor_s* source_string)
{
    ...
    int result = SS$_NORMAL; /* Result */
    unsigned int size_check; // check that haven't gone over
    ...
    size_check = (unsigned int)s1_length + s2_length;
...
// reduce the length so the combined strings will fit
        ...
        {
            memcpy(destination_string->dsc$a_pointer + s1_length, s2_ptr, s2_length);
