...
int lib$sub_times(const struct _generic_64 *time1, const struct _generic_64 *time2, struct _generic_64 *result)
{
    if ((time1->gen64$r_quad_overlay.gen64$b_byte[7] & 0x80) != (time2->gen64$r_quad_overlay.gen64$b_byte[7] & 0x80))
    {
    ...
    else
    {
        ...
        do
        {
            if ((cmp = (time1->gen64$r_quad_overlay.gen64$b_byte[count] - time2->gen64$r_quad_overlay.gen64$b_byte[count])))
                break;
        }
        while (--count >= 0);
        if (cmp < 0)
