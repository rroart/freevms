...
%.lst: %.c
	...
	$(TOPDIR)/linux/scripts/makelst $* $(TOPDIR) $(OBJDUMP)
#
#
#
...
#
fastdep: dummy
	$(TOPDIR)/linux/scripts/mkdep $(CFLAGS) $(EXTRA_CFLAGS) -- $(wildcard *.[chS]) > .depend
ifdef ALL_SUB_DIRS
...
MODINCL = $(TOPDIR)/linux/include/linux/modules
...
$(addprefix $(MODINCL)/,$(export-objs:.o=.ver)): $(TOPDIR)/linux/include/linux/autoconf.h
...
update-modverfile:
	@(echo "#ifndef _LINUX_MODVERSIONS_H";\
	  ...
	  cd $(TOPDIR)/linux/include/linux/modules; \
	  for f in *.ver; do \
	    if [ -f $$f ]; then echo "#include <linux/modules/$${f}>"; fi; \
	  done; \
	  echo "#endif"; \
	) > $(TOPDIR)/linux/include/linux/modversions.h.tmp
	@if [ -r $(TOPDIR)/linux/include/linux/modversions.h ] && cmp -s $(TOPDIR)/linux/include/linux/modversions.h $(TOPDIR)/linux/include/linux/modversions.h.tmp; then \
		echo $(TOPDIR)/linux/include/linux/modversions.h was not updated; \
		rm -f $(TOPDIR)/linux/include/linux/modversions.h.tmp; \
	else \
		echo $(TOPDIR)/linux/include/linux/modversions.h was updated; \
		mv -f $(TOPDIR)/linux/include/linux/modversions.h.tmp $(TOPDIR)/linux/include/linux/modversions.h; \
	fi

$(active-objs): $(TOPDIR)/linux/include/linux/modversions.h
...
ifneq "$(strip $(export-objs))" ""
$(export-objs): $(export-objs:.o=.c) $(TOPDIR)/linux/include/linux/modversions.h
