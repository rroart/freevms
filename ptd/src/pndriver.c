...
int SEARCHDEV(struct _ddb ** ddb,char * string, int strlen)                 // Search for device name
{
    ...
    char loclen;
l10:
    *ddb = (*ddb)->ddb$ps_link;     // Get address of next ddb
...
PZ_OUT_LOOP:
    ...
    {
    case 1:
        goto    PZ_PREMPT;          // Send Prempt Characte - DEC
    case 2:
        goto PZ_STOP;       // Stop output
    case 3:
        goto PZ_CHAR;           // Char in tank
    case 4:
        goto PZ_BURST;          // Burst in progress
    }
//
...
#if 0
static struct _spl SPIN_PN __cacheline_aligned = { spl$l_own_cpu : 0, spl$l_own_cnt :
    -1 /*, spl$l_spinlock : 0*/
};
#else
