...
typedef struct _usg
{
    int owner, flag, rsize, asize, hdr;
};

typedef struct _qb
{
    int flags, uic_long, usage, permquota, overdraft, unused[3];
};
...
{
    static struct _fibdef bfib; /* File information block for bitmap */
    struct
    {
        int fiblen;
    ...
    bfib.fib$b_wsize = 7;
    if (locked == FALSE)
        bfib.fib$l_acctl = FIB$M_WRITE | FIB$M_WRITETHRU | FIB$M_NORECORD | FIB$M_NOLOCK;
    else
    ...
    bytes = 512 * xpages;
    status = SYS$QIOW(0, chan, IO$_READVBLK, &iostat, 0, 0, *addr, bytes, 2, 0, 0, 0);
    if ((status & 1) == 1)
        status = iostat.iosb_1;
    if ((status & 1) != 1)
    {
...

{
    char size;
    register int i;
    int status, bmap_addr[2], count, offset, which, interval, entry, xpages, j;
    unsigned int total_blocks, total_count, lbn, savebit, maxlong, savelongword, largest, temp;
    int *ix;
    ...
    while (i < maxlong) /* Loop through the bitmap longword by longword */
    {
        size = 32 - offset;
        if (*ix == 0)
            status = 0; /* This longword fully used */
        else
            status = lib$ffs(&offset, &size, ix, &offset);
        if ((status & 1) != 1)
        {
        ...
        else
        {
            ...
            while (status != 1)
            {
                ...
                if (*ix == -1)
                    ...
                    status = lib$ffc(&offset, &size, ix, &offset);
...
void new_bitmap(int *addr, struct _fh2 *hdr, int cluster, int curvol, FILE *fp, int pass, int *lis, Boolean *flag,
        Boolean matoutput)
/*
...

{
    ...
    {
        int block[128];
    }*start, *bmap; /* Pointer to a bitmap block (128 longwords */
    struct _hd
    {
    ...
    int i, l, k, ibit, iword, iblock, lbit, lword, lblock, expon, offset, map_bytes, endj, size, lbnc1, lbne1, namelen, format,
            fidnum;
    Boolean report;
    char name[86];
    register int j;
    ...
    i = hdr->fh2$b_idoffset;
    id = (struct _fi2 *) &(head->block[i]);
         ...
         to determine the exact LBN's ('k' loop) */
        {
            ...
            while (j <= endj)
            {
                ...
                if ((lword == iword) && (iblock == lblock))
                    ...
                    size = 32 - ibit; /*How many bits to parse in this longword */
                    ...
                    for (k = j; k <= j + size - 1; k++)
                    {
                        ...
                        if (report == TRUE)
                        {
                            ...
                            if (pass == 2) /* now report the file to the user */
                            {
                                ...
                                sprintf(outbuf,
                                        "%%DFU-E-MULTALLOC, file (%d,%d,%d) %.*s blocks LBN %d through %d multiple allocated",
                                        fidnum, hdr->fh2$w_fid.fid$w_seq, curvol, namelen, name, lbne1, lbnc1);
                                put_disp();
                                if (matoutput)
                                    fprintf(fp, "%s\n", outbuf);
                            }
            ...
            if (report == TRUE)
            {
                ...
                if (pass == 2) /* now report the file to the user */
                {
                    ...
                    fidnum = hdr->fh2$w_fid.fid$b_nmx << 16;
                    fidnum += hdr->fh2$w_fid.fid$w_num;
                    sprintf(outbuf, "%%DFU-E-MULTALLOC, file (%d,%d,%d) %.*s blocks LBN %d through %d multiple allocated", fidnum,
                            hdr->fh2$w_fid.fid$w_seq, curvol, namelen, name, lbne1, lbnc1);
                    put_disp();
                    if (matoutput)
                        fprintf(fp, "%s\n", outbuf);
                }
         ...
         to determine the exact LBN's ('k' loop) */
        {
            ...
            while (j <= endj)
            {
                ...
                else /* We have multiple allocated blocks */
                {
                    ...
                    {
                        i = 512;
                        j = endj + 1; /* Stop further processing */
                        status = 0;
                    }
                    else
                    {
                        ...
                        for (k = j; k <= j + size - 1; k++)
                        {
                            ...
                            if (report == TRUE)
                            {
                                ...
                                if (name[19] != ' ')
                                    memcpy(&name[20], id->fi2$t_filenamext, 66);
                                namelen = strindex(&name[0], " ", 86);
                                sprintf(outbuf, "%%DFU-E-MULTALLOC, file %.*s blocks LBN %d through %d already allocated", namelen,
                                        name, lbne1, lbnc1);
                                put_disp();
                            }
                            ibit++;
            ...
            if (report == TRUE)
            {
                ...
                if (name[19] != ' ')
                    memcpy(&name[20], id->fi2$t_filenamext, 66);
                namelen = strindex(&name[0], " ", 86);
                sprintf(outbuf, "%%DFU-E-MULTALLOC, file %.*s blocks LBN %d through %d already allocated", namelen, name, lbne1,
                        lbnc1);
                put_disp();
            }
        }
...

{
            ...
            if (bounds <= maxblocks) /* Do not cross array bounds */
            {
                ...
                if (chk != -1) /* Found some blocks...*/
                {
                    for (k = 0; k <= 31; k++)
                    {
                        ...
                        if (bounds <= maxblocks) /* Do not cross array bounds */
                        {
                            chk = lib$extzv(&k, &l, &(bmap + i)->block[j]) + lib$extzv(&k, &l, &(start + i)->block[j]);
                            ...
                            else /* Report these blocks */
                            {
                                if (reports == TRUE)
                                {
                                    reports = FALSE;
                                    trigger = TRUE;
                                    sprintf(outbuf, "%%DFU-E-ALLOCSET, blocks LBN %d through %d incorrectly marked free", blkss,
                                            blkse);
                                    put_disp();
                                    if (matoutput)
                                        fprintf(fp, "%s\n", outbuf);
    ...
    if (reports == TRUE)
    {
        reports == FALSE;
        trigger = TRUE;
        sprintf(outbuf, "%%DFU-E-ALLOCSET, blocks LBN %d through %d incorrectly marked free", blkss, blkse);
        put_disp();
        if (matoutput)
            fprintf(fp, "%s\n", outbuf);
        if (rebuild)
    ...
    if (reportc == TRUE)
    {
        reportc == FALSE;
        trigger = TRUE;
        sprintf(outbuf, "%%DFU-E-ALLOCCLR, blocks LBN %d through %d incorrectly marked allocated", blkas, blkae);
        put_disp();
        if (matoutput)
            fprintf(fp, "%s\n", outbuf);
        if (rebuild)
    ...
    if ((rebuild) && (trigger))
    {
        ...
        bytes = 512 * xpages;
        status = SYS$QIOW(0, chan, IO$_WRITEVBLK, &iostat, 0, 0, bmap_addr[0], bytes, 2, 0, 0, 0);
        if ((status & 1) == 1)
            status = iostat.iosb_1;
        if ((status & 1) != 1)
        {
...

{
    ...
    while (!found)
    {
        ...
        else if (usage_table->owner == owner)
            found = TRUE;
        else if (i < 750)
        {
            i++;
...
{
    static struct _fibdef qfib; /* File information block for quota file */
    ...
    status = SYS$QIOW(0, chan, IO$_ACPCONTROL, &iostat, 0, 0, &fibdescr, &qb_descr, &len, &qb_descr, 0, 0);
    if ((status & 1) != 1)
        matquota = FALSE;
    if (iostat.iosb_1 == SS$_QFNOTACT)
        matquota = FALSE;
...
void report_usage(struct _usg *table, short chan, FILE *fp, Boolean matoutput)
/*
...

{
    ...
    {
        status = SYS$FAO(&ctrstr, &id_descr.dsc$w_length, &id_descr, usage_table->owner);
        if ((status & 1) != 1)
            singlemsg(0, status);
        if (matquota == TRUE) /* Get the actual quota */
        {
            ...
            status = SYS$FAO(&ctrstr2, &outlen, &out_descr, &id_descr, usage_table->rsize, usage_table->asize, usage_table->hdr,
        ...
        {
            status = SYS$FAO(&ctrstr3, &outlen, &out_descr, &id_descr, usage_table->rsize, usage_table->asize, usage_table->hdr);
            if ((status & 1) != 1)
                singlemsg(0, status);
        }
        if (matoutput)
...
void check_usage(struct _usg *table, short chan, FILE *fp, Boolean reb, Boolean matoutput)
/*
...

{
    ...
    if (matquota == TRUE)
    {
        ...
        while ((i < 750) && (usage_table->flag == 1) && (ctrlc != 1))
        {
            ...
            {
                SYS$FAO(&ctrstr, &id_descr.dsc$w_length, &id_descr, usage_table->owner);
                sprintf(outbuf, "%%DFU-W-QUOTAERR, %.*s has %d blocks used, QUOTA indicates %d blocks", id_descr.dsc$w_length,
                        ident, exp_quota, qblock.usage);
                put_disp();
                if (matoutput)
                    fprintf(fp, "%s\n", outbuf);
                id_descr.dsc$w_length = 30;
