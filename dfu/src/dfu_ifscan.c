...
extern unsigned int ctrlc, SOR$GK_RECORD, keyb_id, tchan, rows, colls, disp1_id, paste_id, lbnstart, lbncount;
extern Boolean trigger, smg$enable;
extern int ctrlt_ast();
extern char outbuf[255];
globalvalue DFU_NOPRIV, DFU_EXPREG, DFU_ASSIGN, DFU_INDEXERR, DFU_EXHFAIL, DFU_IOERR;
...
static unsigned int lendev, iosize, vbn, headers, highbit;
...
static struct
{
    ...
    {
        { 200, ATR$C_FILE_SPEC, (int *) &dirrec },
        { 0, 0, 0 } }; /* Attribute control block */
...
void cleanup(), copy_fid(), disass_map_ptr(), add_item(), read_indexf_bitmap(), fid_to_name();
...

{
    ...
    int fcnt, efcnt, flgs;
    struct _quad cre_bef, cre_sin, mod_bef, mod_sin, exp_bef, exp_sin, bak_bef, bak_sin, diff;
    int i;
    Boolean matmult, dispfull, matsumm, matplaced, matnovolset, matnosearch, mathome, matstat, matalloc, dummy, matlbn,
            mat_r_minsize, mat_r_maxsize, mat_a_minsize, mat_a_maxsize, matoutput, matname, matexcl, found, matnonebackup,
            matnoneexpired, matbefcreated, matsincreated, matbefmodified, matsinmodified, matbefbackup, matsinbackup, matbefexpired,
            matsinexpired, matsort, matbakfid, matfid, matchars, mator, matmark, matuic, matfragment, matversion;
    int size_min, size_max, parse_type, free_hdr, ivbn, k, j, x, bitblk, bitje, bitval, find, rvn, flags, size, retr_min, retr_max,
            vers_min, vers_max, hdrs, rtvptr, lbn, spacing;
    int parse_item(), read_indexf_multi(), dfu_check();
    unsigned short fid[3];
    unsigned int r_size, a_size, tot_r_size, tot_a_size, fcount, val_headers, fidnum, infid, bakfid, chars, setchar, uic, tothdr,
            totfrag, offset, map_words;
    ...
    status = parse_item("exclude", &dummy_descr, &exclfile[0].name, &matexcl, 3);
    ...
    while ((curvol <= maxvol) && (!ctx.end))
    {
        ...
        if (rvt[curvol].i_open == 1)
        {
            ...
            {
                if (matnovolset == TRUE)
                    curvol = orig_rvn; /*No volume set */
                sprintf(outbuf, "%%DFU-I-SEARCH, Start search on %.*s (%s) , RVN = %d\n", rvt[curvol].devnam_len,
                        rvt[curvol].devnam, &rvt[curvol].fulldevnam[1], curvol);
            }
            put_disp();
            headers = 0;
            val_headers = 0;
            ...
            while (!ctx.end) /* Loop until end of INDEXF or CTRL C entered */
            {
                ...
                if (matfid == TRUE) /* Special case if FID requested */
                {
                    ...
                    status = SYS$QIOW(0, rvt[curvol].channel, IO$_READVBLK, &iostat[(ctx.thread) - 1], 0, 0, &header[ctx.i], 512,
                            vbn, 0, 0, 0);
                }
                ...
                bitval = (headers - 1) % 32;
                status = lib$ffs(&bitval, &size, &bitmap[bitblk + 1].block[bitje * 4], &find);
                ...
                if (dispfull == FALSE)
                {
                    ...
                    {
                        if (spacing > 80)
                            faolen = 86 + spacing * ((dir_descr.dsc$w_length + 46 - 1) / spacing);
                        else
                            faolen = 56 + spacing * ((dir_descr.dsc$w_length + 24 - 1) / spacing);
                        status = SYS$FAO(&ctrstr4, &outlen, &out_descr, faolen, &dir_descr, r_size, a_size);
                    }
                    else
                    {
                        if (spacing > 80)
                            faolen = 76 + spacing * ((dir_descr.dsc$w_length + 56 - 1) / spacing);
                        else
                            faolen = 46 + spacing * ((dir_descr.dsc$w_length + 34 - 1) / spacing);
                        status = SYS$FAO(&ctrstr7, &outlen, &out_descr, faolen, &dir_descr, r_size, a_size, hdrs, rtvptr);
                    }
                    if (matsort == FALSE)
                    {
                        if (matoutput)
...

{
    ...
    int ivbn, bitblk, bitje, bitval, find, rvn, x, fcount, mcount, acount, ccount, dcount, ucount, badrtv, if_frag, bad_r_size,
            bad_a_size, mark_r_size, mark_a_size, mmp_hdr, mmp_blks, r_size, a_size, hdrs, rtvptr, free_hdr, totfrag, tothdr,
    ...
    void get_map_pointers(), add_usage(), report_usage();
    /* FAO parameters */
    unsigned short outlen;
    $DESCRIPTOR(out_descr, outbuf);
    ...
    while ((curvol <= maxvol) && (!ctx.end))
    {
        ...
        if (rvt[curvol].i_open == 1)
        {
            ...
            sprintf(outbuf, "      ***** Volume info for %.*s (from HOME block) *****", rvt[curvol].devnam_len, rvt[curvol].devnam);
            put_disp();
            if (matoutput)
                fprintf(fp, "%s\n", outbuf);
            ...
            if ((status & 1) != 1)
                return (status);
            if (matnoindex)
                ctx.end = TRUE; /*Skip indexf processing */
            while (!ctx.end) /* Loop until end of INDEXF or CTRL C entered */
            {
                hdr = ctx.hdr;
                /* Check corresponding bitmap bit (must be set) */
                ...
                if (headers == 1)
                    if_frag = rtvptr; /* Indexf.Sys fragments */
                if (rtvptr == 1)
                    ccount++;
                else
                {
        ...
        if ((ctrlc != 1) && (!matnoindex))
        {
            ...
            if (matoutput)
                fprintf(fp, "%s\n", outbuf);
            sprintf(outbuf, " INDEXF.SYS fragments/ map_in_use :  %d /%d words ( %d%% used)", if_frag, map_in_use, map_prc);
            put_disp();
            if (matoutput)
                fprintf(fp, "%s\n", outbuf);
            ...
            if (mcount == 0)
                ...
                sprintf(outbuf, " Files marked for delete          :  %d, size %d/%d blocks", mcount, mark_r_size, mark_a_size);
            put_disp();
            if (matoutput)
                fprintf(fp, "%s\n", outbuf);
            ...
            if (matoutput)
                fprintf(fp, "%s\n", outbuf);
            sprintf(outbuf, " Total used/ allocated size       :  %d /%d", tot_r_size, tot_a_size);
            put_disp();
            if (matoutput)
                fprintf(fp, "%s\n", outbuf);
            ...
            if (avefrag < 3.0)
                strcpy(dummy_item, " (fair) ");
            if (avefrag < 2.0)
                strcpy(dummy_item, " (good) ");
            if (avefrag < 1.0)
                strcpy(dummy_item, " (excellent) ");
            sprintf(outbuf, " File fragmentation index         :  %.3f %s", avefrag, dummy_item);
            put_disp();
            if (matoutput)
                fprintf(fp, "%s\n", outbuf);
            ...
            outlen -= x;
            sprintf(outbuf, "    %s%.*s ( %d/%d blocks; %d fragments)\n", &rvt[curvol].fulldevnam[1], outlen, &bfile[x], bad_r_size,
                    bad_a_size, badrtv);
            put_disp();
            if (matoutput)
                fprintf(fp, "%s\n", outbuf);
        ...
        /* Now continue with bitmap processing */
        {
            ...
            if (status == 1)
            {
                ...
                if (matoutput)
                    fprintf(fp, "%s\n", outbuf);
                sprintf(outbuf, " Total blocks on disk             :  %d", rvt[curvol].maxblocks);
                put_disp();
                if (matoutput)
                    fprintf(fp, "%s\n", outbuf);
                ...
                if (matoutput)
                    fprintf(fp, "%s\n", outbuf);
                sprintf(outbuf, " Largest free extent              :  %d  blocks at LBN: %d", largest, large_lbn);
                put_disp();
                if (matoutput)
                    fprintf(fp, "%s\n", outbuf);
                ...
                if (avefrag < 1.0)
                    strcpy(dummy_item, " (excellent) ");
                sprintf(outbuf, " Free space fragmentation index   :  %.3f %s", avefrag, dummy_item);
                put_disp();
                if (matoutput)
                    fprintf(fp, "%s\n", outbuf);
                if (matgraph == TRUE)
                {
                    ...
                    if (matoutput)
                        fprintf(fp, "%s\n", outbuf);
                    sprintf(outbuf, " LBN: 0                                                                %d",
                            rvt[curvol].maxblocks);
                    put_disp();
                    if (matoutput)
                        fprintf(fp, "%s\n", outbuf);
                    tot_a_size = rvt[curvol].maxblocks / 1400;
                    sprintf(outbuf, " Free space distribution; each * = %d free blocks", tot_a_size);
                    put_disp();
                    if (matoutput)
                        fprintf(fp, "%s", outbuf);
                }
...

{
    ...
    int ivbn, bitblk, bitje, bitval, find, rvn, x, j, uic, s_uic, a_size, hdrs, rtvptr, free_hdr, k, size, bytes, fidnum, modifiers,
            fcount, syslost;
    ...
    if (matlist)
        ...
        status = open_device(&device_descr, 1);
    if ((status & 1) != 1)
        return (status);
    ...
    /* Assign a channel for BITMAP, create dynamic space, and read in the BITMAP*/
    {
        ...
        status = read_bitmap(rvt[x].bchan, bmap[x].pagecnt, rvt[x].wlk, &rvt[x].bmap_addr[0]);
        if ((status & 1) != 1)
        {
    ...
    while ((curvol <= maxvol) && (!ctx.end))
    {
        ...
        if (rvt[curvol].i_open == 1)
        {
            if (maxvol == 1)
            {
                sprintf(outbuf, "%%DFU-I-UNDEL, Start search on %.*s (%s)", device_descr.dsc$w_length, device,
                        &rvt[curvol].fulldevnam[1]);
                put_disp();
            }
            else
            {
                sprintf(outbuf, "%%DFU-I-UNDEL, Start search on %.*s (%s) , RVN = %d", rvt[curvol].devnam_len, rvt[curvol].devnam,
                        &rvt[curvol].fulldevnam[1], curvol);
                put_disp();
            }
            headers = 0;
            ...
            while (!ctx.end) /* Loop until end of INDEXF or CTRL C entered */
            {
                ...
                else
                {
                    ...
                    fib.fib$w_nmctl = 0;
                    fib.fib$l_acctl = 0;
                    status = SYS$QIOW(0, rvt[rvn].channel, IO$_ACCESS, &iostat[iocnt], 0, 0, &fibdescr, 0, 0, 0, &acb, 0);
                    status = iostat[iocnt].iosb_1;
                    if ((status & 1) != 1)
                        strcpy(dirrec.dirnam, "[]");
                    else
                ...
                if ((ans[0] == 'y') || (ans[0] == 'Y'))
                {
                    ...
                    for (j = 1; j <= maxvol; j++)
                    {
                        if (rvt[j].bchan != 0)
                        {
                            ...
                            status = SYS$QIOW(0, rvt[j].bchan, IO$_WRITEVBLK, &iostat[iocnt], 0, 0, rvt[j].bmap_addr[0], bytes, 2,
                                    0, 0, 0);
                            if ((status & 1) == 1)
                                status = iostat[iocnt].iosb_1;
                            if ((status & 1) != 1)
                    ...
                    while (found)
                    {
                        ...
                        head->block[255] = 0;
                        for (j = 0; j <= 254; j++)
                            head->block[255] += head->block[j];
                        vbn = rvt[rvn].vbn_file_1 + fidnum - 1;
                        status = SYS$QIOW(0, rvt[rvn].channel, IO$_WRITEVBLK, &iostat[iocnt], 0, 0, &header[x], bytes, vbn, 0, 0,
                                0);
                        if ((status & 1) == 1)
                            status = iostat[iocnt].iosb_1;
                        if ((status & 1) != 1)
                    ...
                    if (qchan != 0)
                        add_quota(qchan, s_uic, hdrs, a_size);
                    sprintf(outbuf, " ");
                    put_disp();
                ...
                /* Next header */
                next_und: status = get_next_header();
                if ((status & 1) != 1)
                    return (status);
            }
        }
    ...
    for (i = 1; i <= maxvol; i++)
        status = SYS$QIOW(0, rvt[i].channel, IO$_DEACCESS, &iostat[iocnt], 0, 0, 0, 0, 0, 0, 0, 0);
    while (first != NULL)
    {
        list = first;
        rvn = list->rvn;
        ...
        status = SYS$QIOW(0, rvt[rvn].channel, IO$_CREATE, &iostat[ctx.thread], 0, 0, &fibdescr, &name_descr, 0, 0, 0, 0);
        if ((status & 1) == 1)
            status = iostat[ctx.thread].iosb_1;
        if ((status & 1) != 1)
        {
            if (syslost == 0)
            ...
            if (syslost == 1) /* Try entering the file in [SYSLOST] */
            {
                ...
                status = SYS$QIOW(0, rvt[1].channel, IO$_CREATE, &iostat[ctx.thread], 0, 0, &fibdescr, &name_descr, 0, 0, 0, 0);
                if ((status & 1) == 1)
                    status = iostat[ctx.thread].iosb_1;
                if ((status & 1) == 1)
                {
            ...
            if (syslost == 2)
            {
                ...
                put_disp();
                singlemsg(0, status);
                sprintf(outbuf, "- Use ANALYZE/DISK/REPAIR to move the file to [SYSLOST]");
                put_disp();
            }
        }
        else
        {
            sprintf(outbuf, "%%DFU-S-ENTERED, File %s entered in original directory", name);
            put_disp();
        }
    }
...

{
    ...
    status = SYS$QIOW(0, l_chan, IO$_ACCESS, &io_stat, 0, 0, &fibdescr, 0, 0, 0, &acb, 0);
    if ((status & 1) == 1)
        status = io_stat.iosb_1;
    if ((status & 1) != 1)
    {
    ...
    else
    {
        ...
        status = SYS$QIOW(0, l_chan, func, &io_stat, 0, 0, &fibdescr, &fname_descr, 0, 0, &acb, 0);
        if ((status & 1) == 1)
            status = io_stat.iosb_1;
        if ((status & 1) != 1)
        {
        ...
        else if (status == SS$_CREATED)
        {
            ...
            status = SYS$QIOW(0, l_chan, IO$_WRITEVBLK, &io_stat, 0, 0, &empty, 2, 1, 0, 0, 0);
        }
    }
    SYS$DASSGN(l_chan);
...

{
    ...
    Boolean multalloc;
    void get_map_pointers(), add_usage(), dfu_handler(), new_bitmap(), check_usage(), report_lost_files();
    /* FAO parameters */
    unsigned short outlen;
    $DESCRIPTOR(out_descr, outbuf);
    $DESCRIPTOR(device_descr, device);
...
#endif
    if (matreb)
        matlock = TRUE; /* Lock anyway if rebuild requested */
    for (i = 0; i < 750; i++)
        usage_table[i].flag = 0;
    /* Get output file */
    clean_flags.fopen = 0;
    ...
    {
        sprintf(outbuf, "%%DFU-W-WRITELK, Cannot rebuild or lock on write-locked device");
        put_disp();
        matlock = FALSE;
        matreb = FALSE;
    }
    ...
    while ((curvol <= maxvol) && (!ctx.end))
    {
        ...
        if (rvt[curvol].i_open == 1)
        {
            if (maxvol == 1)
            {
                sprintf(outbuf, "%%DFU-I-VERIFY, Verifying %.*s (%s) ", device_descr.dsc$w_length, device,
                        &rvt[curvol].fulldevnam[1]);
            }
            else
            {
                sprintf(outbuf, "%%DFU-I-VERIFY, Verifying %.*s (%s) , RVN = %d", rvt[curvol].devnam_len, rvt[curvol].devnam,
                        &rvt[curvol].fulldevnam[1], curvol);
            }
            put_disp();
            headers = 0;
    ...
    if (ctrlc != 1)
        report_lost_files(matfix, matoutput);
            ...
            if (!ctrlc)
            {
                ...
                if (list->function == 2) /* Enter file in SYSLOST */
                {
                    ...
                    if (syslost == 1)
                    {
                        ...
                        strcpy(name, list->name);
                        name_descr.dsc$w_length = strlen(name);
                        status = SYS$QIOW(0, rvt[1].channel, IO$_CREATE, &iostat[ctx.thread], 0, 0, &fibdescr, &name_descr, 0, 0, 0,
                                0);
                        if ((status & 1) == 1)
                            status = iostat[ctx.thread].iosb_1;
                        if ((status & 1) == 1)
...

{
    ...
    for (j = 1; j <= maxvol; j++)
    {
        ...
        for (i = 1; i <= maxi; i++)
        {
            ...
            if (((dyn_array + i)->bitje & 1) != 1)
                goto skip;
            /* Invalid backlink already reported */
            if (((dyn_array + i)->bitje & 2) == 2)
                goto skip;
            ...
            skip: ; /* Next header */
            if (ctrlc == 1)
                i = maxi; /* Dumb method to break off the report */
        } /* End i loop */
        if (ctrlc == 1)
            j = maxvol;
    } /* End j loop */
...

{
    ...
    while ((curvol <= maxvol) && (!ctx.end))
    {
        ...
        if (rvt[curvol].i_open == 1)
        {
            if (maxvol == 1)
            {
                sprintf(outbuf, "%%DFU-I-INDSCAN, Making directory table for %.*s (%s) ", device_descr.dsc$w_length, device,
                        &rvt[curvol].fulldevnam[1]);
            }
            else
            {
                sprintf(outbuf, "%%DFU-I-INDSCAN, Making directory table for %.*s (%s) , RVN = %d", rvt[curvol].devnam_len,
                        rvt[curvol].devnam, &rvt[curvol].fulldevnam[1], curvol);
            }
            put_disp();
            headers = 0;
            ...
            while (!ctx.end) /* Loop until end of INDEXF or CTRL C entered */
            {
                ...
                /* Next header */
                next_file: status = get_next_header();
                if ((status & 1) != 1)
                    return (status);
            }
        }
    ...
    if (ctrlc != 1)
    {
        sprintf(outbuf, "%%DFU-I-DIRSCAN, Scanning %d directories...", dir_cnt);
        put_disp();
    }
    ...
    if ((stat == CLI$_PRESENT) || (stat == CLI$_NEGATED))
    {
        *flag = TRUE;
        ...
        case 0: /* Get the string */
            stat = CLI$GET_VALUE(&item_descr, return_descr, &return_descr->dsc$w_length);
            if ((stat & 1) != 1)
                return (stat);
            break;
        ...
        case 6: /* UIC parsing */
            j = 0;
            stat = CLI$GET_VALUE(&item_descr, &uic_descr, &uic_descr.dsc$w_length);
            uic_x[0] = '\0';
            x = strindex(&uic_s, ",", 30);
            if (x == -1)
            {
 ...
 */
{
    ...
    else /* Get the full file name via an ACP call */
    {
        ...
        fib.fib$w_nmctl = 0;
        fib.fib$l_acctl = 0;
        status = SYS$QIOW(0, rvt[curvol].channel, IO$_ACCESS, &iostat[iocnt], 0, 0, &fibdescr, 0, 0, 0, &acb, 0);
        if ((status & 1) == 1)
            status = iostat[iocnt].iosb_1;
        if ((status & 1) != 1)
        {
        ...
        else
        {
            ...
            if (bakfid == 4)
                ...
                strcpy(ret_dir, &rvt[1].fulldevnam[1]);
            if (x != -1)
                strcat(ret_dir, &dirrec.dirnam[x]);
        }
    }
    if (error)
    {
...

{
    ...
    add_item(&item_list[0].buflen, 64, DVI$_ROOTDEVNAM, &rvt[1].devnam, &rvt[1].devnam_len);
    ...
    add_item(&item_list[7].buflen, 64, DVI$_FULLDEVNAM, &rvt[1].fulldevnam, &rvt[1].fulldevnam_len);
    ...
    if ((devtype == 34) || (devtype == 53) || (devtype == 72))
        ...
        rvt[1].slow = FALSE; /* Decide if we have a CD ROM */
    ...
    {
        strncpy(rvt[1].devnam, device_descr->dsc$a_pointer, device_descr->dsc$w_length);
    ...
    if ((volcount > 1) && (orig_rvn != 1))
    {
        ...
        add_item(&item_list[1].buflen, 64, DVI$_FULLDEVNAM, &rvt[1].fulldevnam, &rvt[1].fulldevnam_len);
    ...
    if ((flag == 1) && (rvt[1].wlk == FALSE))
    {
        ...
        stat = SYS$QIOW(0, lock_chan, IO$_ACPCONTROL, &iostat[0], 0, 0, &fibdescr, 0, 0, 0, 0, 0);
    ...
    while (i <= maxvol)
    {
        ...
        rvt[i].vbn_file_1 = 0;
        if (flag != 2)
            if_fib.fib$w_fid[2] = i; /* Fill in RVN */
        else
            if_fib.fib$w_fid[2] = orig_rvn;

        /* Open the file on ACP level */
        stat = SYS$QIOW(0, rvt[i].channel, func_code, &iostat[0], 0, 0, &fibdescr, 0, 0, 0, 0, 0);
        if ((stat & 1) == 1)
            stat = iostat[0].iosb_1;
        if ((stat & 1) != 1)
        {
        ...
        stat = SYS$QIOW(0, rvt[i].channel, IO$_READVBLK, &iostat[0], 0, 0, &home[i], 512, 2, 0, 0, 0);
        if ((stat & 1) == 1)
            stat = iostat[0].iosb_1;
        if ((stat & 1) != 1)
        {
        ...
        {
            add_item(&item_list[0].buflen, 64, DVI$_NEXTDEVNAM, &rvt[0].devnam, &rvt[0].devnam_len);
            ...
            add_item(&item_list[3].buflen, 64, DVI$_FULLDEVNAM, &rvt[i].fulldevnam, &rvt[i].fulldevnam_len);
 ...
 */
{
    ...
    for (i = 1; i <= iocnt; i++) /* Fire off 2 QIO's */
    {
        ...
        stat = SYS$QIO(efn[i - 1], rvt[curvol].channel, IO$_READVBLK, &iostat[i - 1], 0, 0, &header[(i - 1) * iosize + 1],
 ...
 */
{
    ...
    if (ctx.i == -1) /*start of I/O processing */
    {
        ...
        if (progress_ind)
        {
            sprintf(outbuf, "     Progress : %3d%%     Status : Scanning INDEXF.SYS", progress);
            put_status(1);
        }
        headers = 0;
    ...
    /* now get the next header */
    {
        ...
        if (ctx.i > ctx.endi) /* Next IOSIZE headers */
        {
            if (ctrlc == 1)
                ctx.end = TRUE;
            if (iostat[ctx.thread - 1].length < (iosize * 512))
                ctx.end = TRUE;
            ...
            {
                vbn += iosize;
                status = SYS$QIO(efn[ctx.thread - 1], rvt[curvol].channel, IO$_READVBLK, &iostat[ctx.thread - 1], 0, 0,
                        &header[(ctx.thread - 1) * iosize + 1], (iosize * 512), vbn, 0, 0, 0);
                if ((status & 1) != 1)
                {
 ...
 */
{
    ...
    if (clean_flags.channels == 1)
        for (i = 1; i <= maxvol; i++)
        {
            stat = SYS$QIOW(0, rvt[i].channel, IO$_DEACCESS, &iostat[iocnt], 0, 0, 0, 0, 0, 0, 0, 0);
            stat = SYS$DASSGN(rvt[i].channel);
            rvt[i].channel = 0;
            if (rvt[i].bchan != 0)
            {
                stat = SYS$QIOW(0, rvt[i].bchan, IO$_DEACCESS, &iostat[iocnt], 0, 0, 0, 0, 0, 0, 0, 0);
 ...
 */
{
    ...
    case 1:
        ...
        lbnstart = ((head->block[i] & 0x3f00) << 8) + head->block[i + 1];
        break;
    case 2:
        ...
        break;
    case 3:
        lbncount = ((head->block[i] & 0x3fff) << 16) + head->block[i + 1] + 1;
