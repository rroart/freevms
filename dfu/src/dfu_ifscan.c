...
typedef struct f_id {unsigned short fid_num, fid_seq; 
	             unsigned char fid_rvn, fid_nmx;} ;
typedef struct _it3 { short buflen, itemcode;
	    unsigned int *bufadr; unsigned int *retlen; };
...
struct work { unsigned short fid_num, fid_seq;
		unsigned char fid_rvn, fid_nmx;
		unsigned short did_num, did_seq;
	        unsigned char did_rvn, did_nmx; 
		char name[86]; 
		char function;
		short rvn; struct work *next; } *first, *list;
...
static struct { unsigned int i, starti, endi, thread;
...
static struct _dr { short lendir;
	 char dirnam[198]; } dirrec;
...
static struct { int forward_link, handler_addr, arg_count, condition; } desblk;
...
/* FAO parameters */
  int faolen; unsigned short outlen; 
...
/* Search on allocated filesize */
  ...
  { /* Check minimum size entered */
    ...
    if ((status & 1) != 1) return(status); 
    mat_r_minsize = FALSE; mat_r_maxsize = FALSE;
  ...
  { /* Check minimum size entered */
    ...
    if ((status & 1) != 1) return(status); 
    mat_a_minsize = FALSE; mat_a_maxsize = FALSE;
...
/* Check for specific LBN */
	...
	{ found = FALSE;
          ...
          { disass_map_ptr(hdr,&j);
            ...
            { found = TRUE; j = offset + map_words + 1;
            }
          }
	  if (!found) goto nexti;
...
/* Post the results to user */
	...
	{ x = strindex(&dirrec.dirnam,"]",198); x++;
...

{
  ...
  char line[70];
  struct { int own_uic, flag, rsize, asize, hdr; } usage_table[750];
  void get_map_pointers(),
...
/* Read in index file bitmap, and return free hdrs + highest bit set */
      read_indexf_bitmap(&free_hdr);
      fcount = 0; mcount = 0; acount = 0; ccount = 0; dcount = 0; ucount = 0;
      bad_r_size = 0; bad_a_size = 0; mark_r_size = 0; mark_a_size = 0;
      badrtv = 0; mmp_hdr = 0; mmp_blks = 0; headers = 0; 
      tot_r_size = 0; tot_a_size = 0; tothdr = 0; totfrag = 0;
...
next_rep: status = get_next_header();
    ...
    { sprintf(outbuf,"      ***** File Statistics (from INDEXF.SYS) *****");
      ...
      put_disp(); if (matoutput) fprintf(fp,"%s\n",outbuf);
      a = mmp_hdr; b = mmp_blks / home[curvol].cluster;
...
/* Now continue with bitmap processing */
    { sprintf(outbuf,"     Progress :   0%%     Status : Processing BITMAP.SYS");
      ...
      if ((status & 1) != 1) 
      { singlemsg(DFU_ASSIGN,status); cleanup(); return(status);
      }      
...
/* Now print the graph table */
        ...
        sprintf(outbuf," Free space distribution; each * = %d free blocks",
           tot_a_size);
        put_disp(); if (matoutput) fprintf(fp,"%s",outbuf);
        }
      }
      else 
      { cleanup();
...

{
  ...
  struct ident_area_struct *id;
  struct _hd { unsigned short block[256] ; } *head;
  struct { int pagecnt; } bmap[33];
...
/* Open the volume (set) , read in the home block */
  ...
  { desblk.handler_addr = (int) &dfu_handler;
    ...
    if ((status & 1) !=1) 
    { singlemsg(DFU_EXHFAIL,status); cleanup(); return(status);
    }
  }
...
/* Assign a channel for BITMAP, create dynamic space, and read in the BITMAP*/
  { strcpy(device, rvt[x].devnam);
    ...
    status = SYS$EXPREG(bmap[x].pagecnt, &rvt[x].bmap_addr[0],
      0 , 0);
    if ((status & 1) != 1) 
    { singlemsg(DFU_EXPREG,status); cleanup(); return(status);
    }
    ...
    { sprintf(outbuf,"%%DFU-E-NOTUNDEL, File undeletes not possible,");
      put_disp(); cleanup(); return(status); 
    }
  }
...
/* Now rebuild the mapping pointers into the bitmap.
   ...
   off the whole undelete process */
	    ...
	    { sprintf(outbuf,"%%DFU-E-NOTUNDEL, File cannot be undeleted");
              put_disp();
              ctx.end = TRUE; curvol = maxvol; goto next_und;
            }
            if (hdr->ext_fid_overlay.ext_fid[0] == 0) found = FALSE;
	     else
/* Read in extension header */
...
/* Setup the RVN, FileChar and Checksum fields */
          x = ctx.i; rvn = curvol; found = TRUE; bytes = 512;
	  hdr = (struct header_area_struct *) &header[x];
	  fidnum = hdr->fid_overlay.fid_fields.fid_nmx << 16;
          fidnum += hdr->fid_overlay.fid[0];
	  while(found)
...
/* Check if we are dealing with a directory file */ 
	    ...
	    head = (struct _hd *) hdr; head->block[255] = 0;
...
/* Rewrite the indexf bitmap bit */
	  ...
	  bytes = 512; vbn = home[curvol].ibmapvbn + bitblk;
...
/* Special case for MFD */
	    if (headers == 4) 
	       ...
	       FALSE);
	    list->fid_rvn = curvol; list->next = NULL;
...
{ char fname[14] = "SYSLOST.DIR;1";
  ...
  static struct { unsigned short atr_size, atr_type;
    ...
    acb[5] =  { { 4, ATR$C_UCHAR, (int *) &uchar} , 
	        ...
	        { 2, ATR$C_FPRO, (int *) &fpro}, 
                {4, ATR$C_UIC, (int *) &uic},
      {0, 0, 0} };
  static struct {int fiblen;struct fibdef *fibadr;} fibdescr;
  ...
  if ((status & 1) != 1 )
  { status = SYS$DASSGN(l_chan); return(2);
  }
   else
...

{
  ...
  struct _da { int bakfid;  char rvn, bitje; } *dyn_array;
  struct { int own_uic, flag, rsize, asize, hdr; } usage_table[750];
  struct f_id lost_fid;
  static struct fibdef fib;	/* File information block */
  struct { int fiblen; struct fibdef *fibadr; } fibdescr;
/* Set up list for mutiple allocated blocks */
  struct mult { unsigned int lbnstart,lbnend;
		struct mult *next;} *m_first, *m_list;
  Boolean multalloc;
  void get_map_pointers(), 
...
/* Open the volume (set) , read in the home block */
  ...
  { ctx.i = -1; trigger = FALSE;
    ...
    { if (maxvol == 1)
      ...
      put_disp();
      headers=0;
      m_list = (struct mult *) malloc (sizeof (struct mult)); m_first = m_list;
      m_list->next = NULL;
...
/* Fill in File id */
	    copy_fid(&list->fid_num, &hdr->fid_overlay.fid_fields.fid_num, 
              FALSE);
	    list->fid_rvn = 1;
            list->function = 1; list->next = NULL;
	  }
...
/* Fill in File id */
	    copy_fid(&list->fid_num,&hdr->fid_overlay.fid_fields.fid_num
              , FALSE);
	    list->fid_rvn = curvol;
            list->function = 2; list->next = NULL;
...
/* Fill in File id */
	      ...
	      list->fid_rvn = curvol;
              list->function = 4; list->next = NULL;
	      strncpy(list->name,name,namelen);
            }
	  }
...
{ struct _da { int bakfid;  char rvn, bitje; } *dyn_array, *temp;
...
/* Fill in File id ;  First we remove the old entry 
	...
	list->fid_rvn = 1;
        ...
        list->function = 3; list->next = NULL;
	list->next = (struct work *) malloc(sizeof(struct work));
	list = list->next;
...

{
  ...
  struct _da { short fid[3]; 
               unsigned int bakfid, a_size; 
               char rvn, bitje; } *dyn_array;
...
/* Make sure we use the correct device name on voluem sets */
  ...
  { id = ctx.id;
    ...
    if (tmp != NULL) *tmp = 0;
    strcpy(ret_dir,"[]"); strcat(ret_dir,name); 
...
/* Now loop for all members in volume set */
   ...
   while (i <= maxvol) 
   {
     ...
     rvt[i].i_open = 1; rvt[i].vbn_file_1 = 0; 
    ...
    { if (ctrlc ==1) ctx.end = TRUE;
      ...
      { vbn += iosize;
        status = SYS$QIO(efn[ctx.thread-1],rvt[curvol].channel,IO$_READVBLK,
          ...
          (iosize*512),vbn,0,0,0);
        if ((status & 1) != 1) 
        { singlemsg(DFU_IOERR,status); cleanup();
          return(status);
        }
        (ctx.thread)++; if (ctx.thread > iocnt) ctx.thread = 1;
      }
  ...
  *ptrs = 0;
  ...
  { format = head->block[i] >> 14;
    ...
    switch (format) {
      ...
      case 1: size += (head->block[i] & 255) + 1;
	      break;

      case 2: size += (head->block[i] & 0x3fff) + 1;
	      break;

      case 3: size += ((head->block[i] & 0x3fff) << 16);
...
{ unsigned short format;
  ...
  switch (format) 
    { case 0: lbncount = 0;
	      lbnstart = 0;
	      break;
      case 1: lbncount = (head->block[i] & 255) + 1;
	      lbnstart = ((head->block[i] & 0x3f00) << 8) 
                + head->block[i+1];
	      break;
      case 2: lbncount = (head->block[i] & 0x3fff) + 1;
	      lbnstart = (head->block[i+2] << 16) + head->block[i+1];
	      break;
      case 3: lbncount = ((head->block[i] & 0x3fff) << 16) 
                + head->block[i+1] + 1;
