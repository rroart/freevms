...
typedef struct _d_e
{
    unsigned short size, versionlim;
    unsigned char flags, count, name[];
};
typedef struct _f_e
{
    unsigned short version, fid_num, fid_seq;
...
static Boolean matalias, matcheck, matversion, matdump, mattruncate, mattest, matempty, matrepair, matlbn;
static FILE *fp;
static struct
{
...

{
    ...
    {
        unsigned int date[2];
    } bakdate, expdate, credate, revdate, r_bakdate, r_expdate, r_credate, r_revdate;
    static struct _four
    {
        unsigned short date[4];
    } ascdates;
    static char res_str[255], exp_str[255], devnam[15], fname[255], xname[255], dummy[80], ans[4], ident[30];
    unsigned int func, x, bad, dir, lock, log, cont, rvcount, uic, bucksize, vfcsize, maxrec, eblock, ebyte, hiblk, modifiers;
    unsigned short chan, rtype, fileorg, recsize;
    static unsigned long uchar, old_uic;
    static unsigned char attrib[32];
    Boolean matfile, matbadacl, matdir, matupdate, matlock, matcont, matbakdate, matexpdate, matcredate, matrevdate, matlog,
            matrvcount, matrecsize, matfortran, matimplied, matprint, matnospan, matnone, matbucksize, matvfcsize, matmaxrec,
            matfixed, matvar, matvfc, matundef, matstream, matstreamlf, matstreamcr, matdirect, matindex, matrelat, matseq,
    ...
    status = parse_item("backup_date", &dummy_descr, &r_bakdate, &matbakdate, 4);
    status = parse_item("creation_date", &dummy_descr, &r_credate, &matcredate, 4);
    status = parse_item("expiration_date", &dummy_descr, &r_expdate, &matexpdate, 4);
    status = parse_item("revision_date", &dummy_descr, &r_revdate, &matrevdate, 4);
    ebyte = eblock = 0;
    ...
    while ((matfile) && (ctrlc == 0))/* Loop through the file list */
    {
        ...
        if ((status & 1) != 1)
        {
            sprintf(outbuf, "%%DFU-E-PARSEERR, Error parsing filename from %s,", fname);
            put_disp();
            singlemsg(0, status);
            status = RMS$_NMF;
            goto next_set;
        ...
        while (status != RMS$_NMF)
        {
            ...
            fib.fib$w_fid[0] = nam.nam$w_fid.fid$w_num;
            fib.fib$w_fid[1] = nam.nam$w_fid.fid$w_seq;
            fib.fib$w_fid[2] = nam.nam$w_fid.fid$w_rvn;
            if (matupdate)
                fib.fib$l_acctl =
            ...
            if (matconfirm)
            {
                ...
                if (smg$enable)
                    status = smg$read_composed_line(&keyb_id, 0, &answer, &prompt, &x, &disp1_id, &modifiers, 0, 0, 0, 0, 0);
                else
                    status = smg$read_composed_line(&keyb_id, 0, &answer, &prompt, &x, 0, &modifiers, 0, 0, 0, 0, 0);
                if ((ans[0] == 'a') || (ans[0] == 'A'))
                {
                    matconfirm = FALSE;
                    ans[0] = 'y';
                }
                if ((ans[0] == 'y') || (ans[0] == 'Y') || (ans[0] == '1') || (ans[0] == 't') || (ans[0] == 'T'))
                    ans[0] = 'y';
                else
                    ans[0] = 'n';
            }
            ...
            if (ans[0] == 'y')
            {
                ...
                if (matbadacl)
                    if (bad == 0)
                        uchar = uchar | FH2$M_BADACL; /* Set the bit */
                    else
                        uchar = uchar & ~FH2$M_BADACL; /* Clear the bit */
                if (matdir)
                    if (dir == 0)
                        uchar = uchar | FH2$M_DIRECTORY; /* Set the bit */
                    else
                        uchar = uchar & ~FH2$M_DIRECTORY; /* Clear the bit */
                if (matlock)
                    if (lock == 0)
                        uchar = uchar | FH2$M_LOCKED; /* Set the bit */
                    else
                        uchar = uchar & ~FH2$M_LOCKED; /* Clear the bit */
                if (matcont)
                    if (cont == 0)
                        uchar = uchar | FH2$M_CONTIGB; /* Set the bit */
                    else
                        uchar = uchar & ~FH2$M_CONTIGB; /* Clear the bit */
                if (matbakdate)
                    memcpy(&bakdate, &r_bakdate, 8);
                if (matrevdate)
                    memcpy(&revdate, &r_revdate, 8);
                if (matexpdate)
                    memcpy(&expdate, &r_expdate, 8);
                if (matcredate)
                    memcpy(&credate, &r_credate, 8);
                if (matebyte)
                    memcpy(&attrib[12], &ebyte, 2);
                if (mateblock)
                {
                    if (!matebyte)
                    {
                ...
                if (matuic)
                    old_uic = uic;
                if (matrvcount)
                    ascdates.date[0] = rvcount;
                if (matrecsize)
                    memcpy(&attrib[2], &recsize, 2);
                if (matbucksize)
                    attrib[14] = (unsigned char) bucksize;
                if (matvfcsize)
                    attrib[15] = (unsigned char) vfcsize;
                if (matmaxrec)
                    attrib[16] = (unsigned char) maxrec;
                if (matnone)
                    attrib[1] = 0;
                if (matfortran)
                    attrib[1] = FAT$M_FORTRANCC;
                if (matimplied)
                    attrib[1] = FAT$M_IMPLIEDCC;
                if (matprint)
                    attrib[1] = FAT$M_PRINTCC;
                if (matnospan)
                    attrib[1] = attrib[1] | FAT$M_NOSPAN;
                if (matfixed)
                    rtype = FAT$C_FIXED;
                if (matvar)
                ...
                if (matstream)
                    rtype = FAT$C_STREAM;
                if (matstreamlf)
                    rtype = FAT$C_STREAMLF;
                if (matstreamcr)
                    rtype = FAT$C_STREAMCR;
                if (matindex)
                    fileorg = FAT$C_INDEXED;
                if (matdirect)
                    fileorg = FAT$C_DIRECT;
                if (matrelat)
                    fileorg = FAT$C_RELATIVE;
                if (matseq)
                    fileorg = FAT$C_SEQUENTIAL;
                attrib[0] = (unsigned char) (fileorg * 16) + (unsigned char) rtype;
                /* Now rewrite the file header */
                status = SYS$QIOW(0, chan, IO$_MODIFY, &iostat, 0, 0, &fibdescr, 0, 0, 0, acb, 0);
                if ((status & 1) == 1)
                    status = iostat.iosb_1;
                if ((status & 1) != 1)
                {
            ...
            status = SYS$QIOW(0, chan, IO$_DEACCESS, &iostat, 0, 0, &fibdescr, 0, 0, 0, 0, 0);
            status = 1;

            next_set: if ((status & 1) == 1)
                status = SYS$SEARCH(&fab, 0, 0);
            else
                status = RMS$_NMF;
            SYS$DASSGN(chan);
...

{
    ...
    {
        status = SYS$QIOW(0, dchan, func, &iostat, 0, 0, &dfib_descr, 0, 0, 0, &d_acb, 0);
        if ((status & 1) == 1)
            status = iostat.iosb_1;
        if ((status & 1) != 1)
        {
            if (status != SS$_NOSUCHFILE)
                ...
                status2 = SYS$QIOW(0, dchan, IO$_ACCESS, &iostat, 0, 0, &dfib_descr, 0, 0, 0, &d_acb, 0);
            tmp = strindex(&file, " ", 86);
            file[tmp] = '\0';
        ...
        if ((uchar & 32768) == 32768)
            noremove = TRUE; /* Marked bit */
    ...
    status = SYS$QIOW(0, dchan, func, &iostat, 0, 0, &dfib_descr, 0, 0, 0, 0, 0);
    if ((status & 1) == 1)
        status = iostat.iosb_1;
    if ((status & 1) != 1)
    {
    ...
    status = 1;
    end_del: return (status);
}

int remove_file(unsigned short id[3], unsigned short did[3], unsigned short dchan)
/*
...
int delete_directory(unsigned int dchan, unsigned short f_id[3], char *r_str, Boolean nolog)
/*
...
{
    unsigned int func;
    int i, j, k, n, nr, vbn, x, e_blk, h_blk, progress, num_files, lastsize;
    static unsigned int uchar;
    ...
    {
        char block[512];
    } empty, *i_dir; /* Pointer to shared memory */
    Boolean found;
    ...
    if (e_blk > 1000)
    {
        ...
        if ((status & 1) != 1)
        {
            sprintf(outbuf, "%%DFU-E-INSFMEM, Error allocating %d pages memory for %s", e_blk, r_str);
            put_disp();
            singlemsg(0, status);
            lastsize = 0;
            status = SYS$QIOW(0, dchan, IO$_DEACCESS, 0, 0, 0, 0, 0, 0, 0, 0, 0);
    ...
    status = SYS$QIOW(0, dchan, IO$_READVBLK, &iostat, 0, 0, i_dir, (vbn * 512), 1, 0, 0, 0);
    if ((status & 1) == 1)
        status = iostat.iosb_1;
    if ((status & 1) != 1)
    {
    ...
    if (i_dir->block[0] == 0)
    {
        sprintf(outbuf, "%%DFU-E-JUNKDIR, Corrupted directory (byte offset 0) %s", r_str);
        put_disp();
        status = SYS$QIOW(0, dchan, IO$_DEACCESS, 0, 0, 0, 0, 0, 0, 0, 0, 0);
        goto next_del;
    }
    ...
    while (k < e_blk)
    {
        ...
        sprintf(outbuf, "     Progress : %3d%%     Status : Deleting directory", progress);
        put_status(1);
        i = 1;
        found = FALSE;
        while (!found)
        {
            if ((i > 512) || i_dir->block[i - 1] == -1)
                found = TRUE; /* End of block */
            ...
            /* Pointer to directory record */
            {
                ...
                while (nr >= 1)
                {
                    ...
                    if ((status & 1) == 1)
                    {
                        ...
                        if (!nolog)
                        {
                            sprintf(outbuf, "%%DFU-S-DELETED, File %.*s;%d deleted ", dir_entry->count, dir_entry->name,
                                    fid_entry->version);
                            put_disp();
                        }
                    }
    ...
    attrib[5] = 2;
    vbn = 1;
    status = SYS$QIOW(0, dchan, IO$_WRITEVBLK, &iostat, 0, 0, &empty.block[0], 2, 1, 0, 0, 0);
    if ((status & 1) == 1)
        status = iostat.iosb_1;
    if ((status & 1) != 1)
        status = SYS$QIOW(0, dchan, IO$_DEACCESS, 0, 0, 0, 0, 0, 0, 0, 0, 0);
    ...
    /* Write the new attributes back , truncate and close the file */
    {
        ...
        status = SYS$QIOW(0, dchan, IO$_DEACCESS, &iostat, 0, 0, &fib_descr, 0, 0, 0, &acb, 0);
    }
    status = delete_file(f_id, dchan, FALSE, FALSE, nolog, FALSE);
    next_del: if (lastsize != 0)
...

{
    ...
    while (status != RMS$_NMF)
    {
        if ((status & 1) != 1)
        {
            ...
            for (x = xnam.nam$b_rsl; x <= 254; x++)
                sort_rec.rx_str[x] = ' ';
        sort_rec.f_id[0] = xnam.nam$w_fid.fid$w_num;
        sort_rec.f_id[1] = xnam.nam$w_fid.fid$w_seq;
        sort_rec.f_id[2] = xnam.nam$w_fid.fid$w_rvn;
#if 1
    ...
    {
        x = strindex(sort_rec.rx_str, " ", 255);
        sort_rec.rx_str[x] = '\0';
        if (ctrlc != 1)
        {
            sprintf(outbuf, "\n%%DFU-I-CLEANUP, Deleting %s...", sort_rec.rx_str);
            put_disp();
            status = delete_directory(dchan, &sort_rec.f_id[0], (char *) &sort_rec.rx_str, nolog);
#if 1
            status = sor$return_rec(&sort_descr);
#endif
        }
        else
            status = SS$_ENDOFFILE; /* Stop at CTRL/C */
    }
#if 1
    sor$end_sort(0);
#endif
...
{
    static char device[64], dummy[7], dname[160], exp_str[255], res_str[255], t_str[255];
    unsigned long tmp;
    struct FAB fab;
    struct NAM nam;
    static struct
    {
        unsigned short num, seq;
        unsigned char rvn, nmx;
    } f_id;
    unsigned fid_num, x;
    unsigned short chan;
    ...
    if (!matdir)
    {
        ...
        {
            sprintf(outbuf, "%%DFU-E-NOFID, Missing /FILE_ID or /DIRECTORY qualifier on command line");
            put_disp();
            return (SS$_NOSUCHFILE);
        }
    ...
    /* Delete all files in this directory */
    {
        ...
        while (status != RMS$_NMF)
        {
            ...
            strcpy(t_str, res_str);
            status = 1;
            if (mattree)
                status = parse_tree(chan, (char *) &t_str, matnolog);
            if ((status & 1) == 1)
            {
                if (ctrlc != 1)
                {
                    ...
                    put_disp();
                    status = delete_directory(chan, &nam.nam$w_fid.fid$w_num, (char *) &res_str, matnolog);
                }
                if (ctrlc == 1)
                {
                    do_abort();
    ...
    put_disp();
    if (matstat)
        status = lib$show_timer(0, 0, display_stat, 0);
}
...
{
    int x, y, flag;
    static char device[64], dummy_item[80], fname[86], version[8], dfile[255], file[160];
    struct
    {
    ...
    if (x > 0)
    {
        sprintf(outbuf, "%%DFU-E-ILLIOFUNC, This function not supported on this version of VMS");
        put_disp();
        return (1);
    }
...
int move_to_lbn(unsigned short * from, unsigned int lbn_to, unsigned short chan)
/*
...

{
    ...
    /* Now defrag the file */
    status = SYS$QIOW(0, chan, func, &iostat, 0, 0, &fibdescr, 0, 0, 0, 0, 0);
    if ((status & 1) == 1)
        status = iostat.iosb_1;
    return (status);
}
...

{
    ...
    while (status != RMS$_NMF)
    {
        ...
        if (matlbn)
        {
            status = move_to_lbn(&nam.nam$w_fid.fid$w_num, lbn, chan);
        }
        else
        {
            /* Now fill in File id and function code for the ACP call */
            m_fib.fib$w_fid[0] = nam.nam$w_fid.fid$w_num;
            m_fib.fib$w_fid[1] = nam.nam$w_fid.fid$w_seq;
            m_fib.fib$w_fid[2] = nam.nam$w_fid.fid$w_rvn;
...
{
    static char fname[80], dname[160], dummy_item[80], device[64], dfile[255], file[160];
    int x, build_dir_table();
    FILE *ip;
    Boolean matstat, matcompress, dummy, matoutput, matdevice, matcreate, matalloc, matratio;
    void create_dir();
    $DESCRIPTOR(dname_descr, dname);
    $DESCRIPTOR(dummy_descr, dummy_item);
    ...
    if (!matcreate && !matdump && !matcompress && !matversion && !matalias && !matempty && !matcheck)
    {
    ...
    /* Follow different code path for this function */
    {
        ...
        if (matstat)
            lib$show_timer(0, 0, display_stat, 0);
        fclose(fp);
 ...
 */
{
    ...
    static struct
    {
        ...
        {
            { ATR$S_UCHAR, ATR$C_UCHAR, (long) &uchar },
            { 0, 0, 0 } };
    ...
    if (status & 1)
        status = SYS$QIOW(0, channel, IO$_DEACCESS, &iostat, 0, 0, &fibdescr, 0, 0, 0, &acb, 0);
    if ((status & 1) == 1)
        status = iostat.iosb_1;
    if ((status & 1) != 1)
    {
    ...
    {
        sprintf(outbuf, "%%DFU-S-CREATED, Created directory %.*s", nam.nam$b_rsl, res_str);
        put_disp();
    }
}
...

{
    ...
    {
        char block[512];
    }*i_dir, *o_dir; /* Pointer to shared memory */
    $DESCRIPTOR(devnam_descr, devnam);
    ...
    while (status != RMS$_NMF)
    {
        ...
        if (e_blk > 1000)
        {
            ...
            if ((status & 1) != 1)
            {
                sprintf(outbuf, "%%DFU-E-INSFMEM, Error allocating %d pages memory for %s", e_blk, res_str);
                put_disp();
                singlemsg(0, status);
                lastsize = 0;
                goto next_dir;
        ...
        if (ratio != 100)
        {
            ...
            if ((status & 1) != 1)
            {
                sprintf(outbuf, "%%DFU-E-INSFMEM, Error allocating %d pages memory ", h_blk);
                put_disp();
                singlemsg(0, status);
                goto next_dir;
            }
        ...
        if (ratio != 100)
            ...
            o_dir = i_dir;
        status = SYS$QIOW(0, chan, IO$_READVBLK, &iostat, 0, 0, i_dir, (vbn * 512), 1, 0, 0, 0);
        if ((status & 1) == 1)
            status = iostat.iosb_1;
        if ((status & 1) != 1)
        {
        ...
        if (i_dir->block[0] == 0)
        {
            sprintf(outbuf, "%%DFU-E-JUNKDIR, Corrupted directory (byte offset 0) %s", res_str);
            put_disp();
            goto next_dir;
        }
        ...
        while (k < e_blk)
        {
            ...
            while (!found)
            {
                ...
                else /* parse this entry */
                {
                    ...
                    {
                        sprintf(outbuf, "%%DFU-E-LOWFILL, Directory entry too large, choose a higher fill factor");
                        put_disp();
                        goto next_dir;
                    }
                    ...
                    if (matdump)
                    {
                        sprintf(outbuf, "Size: %d, Version limit: %d, Type: %d, Name(%d): %.*s", dir_entry->size,
                                dir_entry->versionlim, dir_entry->flags, dir_entry->count, dir_entry->count, dir_entry->name);
                        put_disp();
                        if (matoutput)
                            fprintf(fp, "%s\n", outbuf);
                        /* Align count field */
                        ...
                        while (nr >= 1)
                        {
                            ...
                            sprintf(outbuf, "   Version: %d, FID : (%d,%d,%d)", fid_entry->version, fid_num, fid_entry->fid_seq,
                                    fid_entry->fid_rvn);
                            put_disp();
                            if (matoutput)
                                fprintf(fp, "%s\n", outbuf);
                    ...
                    if (newi > h_blk)
                    {
                        ...
                        put_disp();
                        sprintf(outbuf, " Choose a higher fill factor");
                        put_disp();
                        goto next_dir;
                    }
        ...
        /* Rewrite the new , compressed directory */
        {
            ...
            status = SYS$QIOW(0, chan, IO$_WRITEVBLK, &iostat, 0, 0, o_dir, (vbn * 512), 1, 0, 0, 0);
            if ((status & 1) == 1)
                status = iostat.iosb_1;
            if ((status & 1) != 1)
            {
            ...
            status = SYS$QIOW(0, chan, IO$_DEACCESS, &iostat, 0, 0, &fibdescr, 0, 0, 0, &acb, 0);
            if (mattruncate)
                e_blk = fib.fib$l_exvbn - 1;
            else
                e_blk = h_blk;
        }
        if (mattest)
            sprintf(outbuf, "%%DFU-I-TESTONLY, %s : %d files; old size : %d/%d, new : %d blocks", res_str, n, oldeblk, h_blk, newi);
        else if (matdump)
            sprintf(outbuf, "%%DFU-I-TOTAL, %s : %d files", res_str, n);
        else
            sprintf(outbuf, "%%DFU-S-DONE, %s : %d files; was : %d/%d, now : %d/%d blocks", res_str, n, oldeblk, h_blk, newi,
                    e_blk);
        put_disp();
...

{
    ...
    static struct
    {
        ...
        {
            { 32, ATR$C_RECATTR, (long) &attrib },
            { 200, ATR$C_FILE_SPEC, (long) &dirrec },
            { 0, 0, 0 } }, acb_fil[3] =
        {
            { 6, ATR$C_BACKLINK, (long) &did.fid_num },
            { 200, ATR$C_FILE_SPEC, (long) &filrec },
            { 0, 0, 0 } };
    ...
    while ((l <= maxi) && (ctrlc != 1))
    {
        ...
        /* Open the directory */
        status = SYS$QIOW(0, (rvt_t + rvn)->channel, func, &iostat, 0, 0, &fibdescr, 0, 0, 0, &acb, 0);
        if ((status & 1) == 1)
            status = iostat.iosb_1;
        if ((status & 1) != 1)
            goto next;
        ...
        if (e_blk <= 1000)
            ...
            i_dir = (struct _sh *) retadr.address[0];
        status = SYS$QIOW(0, (rvt_t + rvn)->channel, IO$_READVBLK, &iostat, 0, 0, i_dir, (vbn * 512), 1, 0, 0, 0);
        SYS$QIOW(0, (rvt_t + rvn)->channel, IO$_DEACCESS, 0, 0, 0, 0, 0, 0, 0, 0, 0);
        if ((status & 1) == 1)
            status = iostat.iosb_1;
        if ((status & 1) != 1)
        ...
        *tmp = '.';
        ...
        if ((dir_cnt + cnt) % 10 == 0)
        {
            ...
            sprintf(outbuf, "     Progress : %3d%%     Status : Scanning directories", progress);
            put_status(1);
        }
        strcpy(oldnam, "\0");
        dir_cnt++;
        ...
        if (matempty && (!matalias && !matversion && !matcheck))
            goto next;
        newi = 1;
        k = 1;
        n = 0;
        ...
        while (k < e_blk)
        {
            ...
            if (i_dir->block[0] == 0)
            {
                sprintf(outbuf, "Directory corrupt %s%s first byte , block %d ", fulldevnam, &dirrec.dirnam, k);
                put_disp();
                if (matoutput)
                    fprintf(fp, "%s\n", outbuf);
                goto next;
            ...
            while (!found)
            {
                ...
                else /* parse this entry */
                {
                    ...
                    if (matcheck)
                    {
                        ...
                        if (x > 0)
                        {
                            sprintf(outbuf, "%s%s%s out of alphabetical order", fulldevnam, &dirrec.dirnam, &oldnam);
                            put_disp();
                            if (matoutput)
                                fprintf(fp, "%s\n", outbuf);
                        }
                        ...
                        while (n > 0)
                        {
                            ...
                            {
                                sprintf(outbuf, "%s%s%.*s;%d has no valid file header", fulldevnam, &dirrec.dirnam,
                                        dir_entry->count, dir_entry->name, fid_entry->version);
                                put_disp();
                                if (matrepair) /* Remove this entry */
                                {
                                    status = remove_file(&fid_entry->fid_num, &fib.fib$w_fid[0], (rvt_t + xrvn)->channel);
                                    if ((status & 1) != 1)
                                    {
                                        sprintf(outbuf, "%%DFU-E-NOREMOVE, error removing %.*s", dir_entry->count, dir_entry->name);
                                        put_disp();
                                        singlemsg(0, status);
                                    }
                                    else
                                    {
                                        sprintf(outbuf, "%DFU-S-REMOVED, file %.*s removed", dir_entry->count, dir_entry->name);
                                        put_disp();
                                    }
                                }
                            }
                            else if (temp->fid[1] != fid_entry->fid_seq) /*Invalid seq*/
                            {
                                sprintf(outbuf, "%s%s%.*s;%d invalid file sequence number", fulldevnam, &dirrec.dirnam,
                                        dir_entry->count, dir_entry->name, fid_entry->version);
                                put_disp();
                            }
                            n--;
                    ...
                    if (matversion)
                    {
                        ...
                        else
                        {
                            ...
                            if (x != 0) /* report previous entry */
                            {
                                if (nr >= version)
                                {
                                    sprintf(outbuf, "%s%s%s , %d versions , %d blocks", fulldevnam, &dirrec.dirnam, &savnam, nr,
                                            totsize);
                                    put_disp();
                                    if (matoutput)
                                        fprintf(fp, "%s\n", outbuf);
                        ...
                        /* Report this file */
                        {
                            sprintf(outbuf, "%s%s%.*s , %d versions , %d blocks", fulldevnam, &dirrec.dirnam, dir_entry->count,
                    ...
                    /* Setup fid pointer for alias check */
                    {
                        ...
                        while (n > 0)
                        {
                            ...
                            /* Set up FIB to do a pseudo FID_TO_NAME call */
                            {
                                ...
                                fib.fib$w_fid[2] = (fid_entry->fid_nmx << 8) + fid_entry->fid_rvn;
                                status = SYS$QIOW(0, (rvt_t + xrvn)->channel, IO$_ACCESS, &iostat, 0, 0, &fibdescr, 0, 0, 0,
                                        &acb_fil, 0);
                                if ((status & 1) == 1)
                                    status = iostat.iosb_1;
                                if ((status & 1) != 1)
                                 ...
                                 the dynamic array (because of concurrent disk activity). */
                                {
                                    if (((did.fid_nmx << 16) + did.fid_num) != l)
                                    {
                                        ...
                                        sprintf(outbuf, "%s%s%.*s;%d is alias for %s%s", fulldevnam, &dirrec.dirnam,
                                                dir_entry->count, dir_entry->name, fid_entry->version, fulldevnam, &filrec.dirnam);
                                        put_disp();
                                        if (matoutput)
                                            fprintf(fp, "%s\n", outbuf);
                                    }
        ...
        if ((flag) && (nr >= version))
        {
            flag = FALSE;
            /* Report this file */
            sprintf(outbuf, "%s%s%.*s , %d versions , %d blocks", fulldevnam, &dirrec.dirnam, dir_entry->count, dir_entry->name, nr,
                    totsize);
            put_disp();
            if (matoutput)
                fprintf(fp, "%s\n", outbuf);
